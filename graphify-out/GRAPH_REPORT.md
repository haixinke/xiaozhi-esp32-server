# Graph Report - xiaozhi-esp32-server  (2026-08-29)

## Corpus Check
- 1651 files · ~684,494 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 14208 nodes · 38686 edges · 641 communities (415 shown, 226 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 2972 edges (avg confidence: 0.75)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d798dc10`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Result
- index-Guo1hQ-y.js
- SysUserTokenEntity
- BaseService
- AgentSnapshotServiceImplTest.java
- getServiceUrl
- ActionResponse
- TTSProvider
- lombok.Data
- CompanionServiceImplTest
- StoryEngineController.java
- AsyncConfig.java
- AgentSnapshotPanel.vue
- ConnectionHandler
- KnowledgeBaseServiceImpl
- PdcNfcJobStateMachineTest
- aA
- StoryEvaluationResult
- httpRequest.js
- connection.py
- org.junit.jupiter.api.DisplayName
- edit.vue
- AgentChatHistoryBizServiceImpl
- .push
- .updateFailureIsReturnedToCaller
- t
- lombok.RequiredArgsConstructor
- nQ
- settings/index.vue
- xiaozhi-server/app.py
- kl
- pet-store.js
- lombok.AllArgsConstructor
- MCPClient
- egg-miniprogram/miniprogram/app.js
- index/index.js
- .get
- TTSProviderBase
- PetServiceImplTodayMoodTest
- TextMessageType
- PdcNfcProperties
- test_chat_integration.py
- devDependencies
- change-soul.js
- doodle-api.js
- register/index.vue
- home.js
- InviteServiceImplTest
- AgentChatHistoryService
- PdcNfcInventoryServiceImpl
- .generate
- tools.vue
- get
- get
- Alova HTTP Client
- DeviceServiceImpl
- doodle-editor.js
- dependencies
- ASRProvider
- J
- MemoryProviderBase
- is
- PaymentOrderEntity
- PetServiceImplStoryContextTest
- device/index.vue
- PdcNfcWriteResultImporterTest
- roleConfig.vue
- voice-call.js
- MoodDeciderTest
- org.springframework.web.socket.WebSocketSession
- AgentSnapshotDialog.vue
- org.apache.ibatis.annotations.Mapper
- scripts
- utils/index.ts
- CorrectWordFileService
- PdcNfcManualWriteServiceTest
- TestRuntimeApplication
- i
- detail.vue
- .sourceToTarget
- AgentSnapshotServiceImpl
- ProviderManagement.vue
- manager-api
- ContentSafetyGate
- utils_vad.py
- error
- home.vue
- pet-store.test.js
- PdcNfcWriteJobServiceTest
- PrototypeStoryStateServiceImplTest
- main/miniprogram/app.js
- Constant.java
- MicrophoneListener
- chat.js
- collection-card.js
- PromptManager
- org.springframework.transaction.annotation.Transactional
- ultrasonic-config.vue
- home.test.js
- .decide
- KnowledgeBaseManagement.vue
- SafetyResult
- PetStoryQueryServiceImplTest
- compilerOptions
- le
- AgentSnapshotField
- org.springframework.context.annotation.Configuration
- RAGFlowAdapter
- MCPEndpointClient
- pages
- ReplacementWordManagement.vue
- .isEnabled
- BigSceneList.vue
- GIFEncoder
- index.test.js
- AgentEntity
- StoryActionImageServiceImpl.java
- ya
- .getUserId
- backpack.js
- WebSocketManager
- MqttGatewayAuthorizationTest
- NfcWriteJobManagement.vue
- my.js
- AgentSnapshotServiceImplTest
- WebSocketManager
- NfcSchemeManagement.vue
- CompanionServiceImpl
- DeviceAddressBookServiceImpl
- ActionList.vue
- BaseServiceImplTest
- DeviceManagement.vue
- pages
- login/index.vue
- lombok.extern.slf4j.Slf4j
- StoryStateSelectorTest
- ft_render-ByO_jG18.js
- AudioPlayer.vue
- agent/index.vue
- com.baomidou.mybatisplus.annotation.TableName
- feedback/feedback.js
- voice-call.test.js
- GlobalCacheManager
- XssHttpServletRequestWrapper
- Builder
- DictManagement.vue
- PdcNfcAdminRequestEntity
- HeaderBar.vue
- WakewordDetector
- ZA
- ModelEditDialog.vue
- VoiceCloneDialog.vue
- VoiceCallManager
- XssFilter.java
- doodle-canvas.js
- incubation-scene.js
- incubation-environment.js
- environment-state.js
- ModelConfig.vue
- destiny.js
- FeedbackServiceImpl
- 小智后台管理系统（Manager API）
- FunctionDialog.vue
- daily-window-detail.js
- manager-mobile/package.json
- TtsModel.vue
- pdcNfcState.mjs
- PageData
- test_aliyun_provider.py
- org.springframework.context.annotation.Bean
- doodle-canvas.test.js
- ConfigService
- orders.js
- audio.test.js
- 核心能力提供商适配层（ASR/TTS/LLM/Memory/Intent/VAD/VLLM）
- devDependencies
- .beforeQuery
- RuntimeConfig
- dependencies
- Plugin
- VoicePrint.vue
- PdcNfcClaimController
- ServerSecretFilter
- AgentVoicePrintServiceImpl
- VoiceCloneServiceImpl
- PdcNfcMetrics
- UserManagement.vue
- room-clock.js
- TTSProvider
- fg-tabbar.vue
- AgentSnapshotEntity
- as
- lI
- ItemServiceImpl
- CustomDialog.vue
- manifest.json
- EventEmitter
- buildDiffs
- FeedbackService
- PdcNfcWriteCsvExporterTest
- NfcBatchManagement.vue
- PetStoryStateEntity
- pages/profile/profile.js
- applyTheme
- pet-store-actions.test.js
- permissions
- ChatHistoryDialog.vue
- subscription.js
- AudioManager
- FeatureManager
- WakewordEventBridge
- KnowledgeBaseAdapter
- DateUtils
- WechatPayClient
- FeatureManagement.vue
- registerType
- TTSProvider
- lombok.NoArgsConstructor
- ServerMCPClient
- SimpleHttpServer.start
- agentSnapshotDisplayUtils.mjs
- setup_logging
- verify-project.js
- IntimacyLevel.java
- NfcAssetManagement.vue
- .register
- org.apache.shiro.authc.AuthenticationToken
- Selected Module Pipeline
- WakeupWordsConfig
- HatchActionServiceImplTest
- StreamTTSPerformanceTester
- app.test.js
- VoiceCloneManagement.vue
- account.test.js
- time-service.js
- LLMProvider
- Vue CLI Build Configuration
- ASRProvider
- PdcNfcClaimPreviewServiceTest
- NfcActivationScanner.vue
- add-device.test.js
- window-weather-canvas.js
- SubscriptionServiceImpl
- mp-weixin
- buildSavedVersionDiffSnapshot
- remote-image.test.js
- PetSystemPromptTemplate
- Multi-Level Memory
- OtaEntity
- _common.sh
- invite-codes.test.js
- profile.test.js
- .validateEntity
- ASRProvider
- PetMood
- WechatPhoneGate
- ProviderDialog.vue
- TTSProvider
- PaymentNotifyController.java
- Agent Base Prompt Template
- PdcNfcSchemeAdminController
- chat_integration_review_cases.py
- java.lang.annotation.Documented
- HatchActionType
- ContextProviderDTO
- com.fasterxml.jackson.annotation.JsonIgnoreProperties
- alova.ts
- ChatHistoryExportServiceImpl
- daily-window-detail.test.js
- app-plus
- createWasm
- AgentChatHistoryServiceImpl
- TtsAdvancedSettings.vue
- AgentChatSummaryServiceImpl
- AgentTemplateManagement.vue
- Subscription Page (我的契约)
- nfc-claim.test.js
- ShiroConfig.java
- chat.test.js
- welcome.test.js
- CaptchaServiceImpl
- ASRProvider
- GIF
- manager-web/package.json
- LLMPerformanceTester
- floating-call-ball.js
- ._record_audit
- 微信支付改造三道防线
- incubation-scene.test.js
- collection-card.test.js
- my.test.js
- ipad
- OpusEncoderUtils
- voice-call-manager.js
- VoiceCloneService
- VoiceResourceManagement.vue
- pet-mood-tab.js
- NfcProductTypeManagement.vue
- doodle-save.test.js
- .createAgent
- distribute
- iphone
- ccall
- check-i18n.js
- functionParamRows
- date.js
- router/index.js
- post
- DeviceEntity
- PdcNfcWriteJobDownloadTest
- android
- vue.config.js
- AgentMcpAccessPointServiceImpl
- KnowledgeFilesController
- websocket Utility
- SAE Startup Wrapper Script
- intentHandler.py
- UploadScene.java
- fetchSnapshots
- scripts
- WechatAccessTokenProviderImplTest
- VADProviderBase
- DeviceTypeRegistry
- deregister.js
- AgentVoicePrintVO
- TestRuntimeHttpServer
- ASRProvider
- lombok.Getter
- XssUtils
- SmallSceneList.vue
- submitRestoreSnapshot
- callRuntimeCallbacks
- normalizeValueForField
- SysParamsServiceImpl
- org.junit.jupiter.api.Test
- ServerActionEnum
- nfc-claim.js
- service-worker.js
- p3.py
- performance_tester.py
- 项目文档与部署指南
- egg-avatar.js
- provider.vue
- AdminApplication
- NfcOperationLogManagement.vue
- postupgrade.js
- typings.d.ts
- emscripten_realloc_buffer
- Chinese FST Commands
- OpenAI LLMProvider.response
- UnifiedToolHandler._initialize
- MCP Server Settings Template
- OpusEncoderUtils.encode_pcm_to_opus_stream
- Live2D Cubism 示例模型
- wakeword_runtime/__init__.py
- mood-badge.js
- LLMProvider
- manager-mobile pnpm workspace
- manifest.config.ts
- @uni-helper/unocss-preset-uni
- typings.ts
- manager-web/docker/start.sh
- manager-web Vue 应用壳
- org.junit.jupiter.api.BeforeEach
- ContextProviderDialog.vue
- Gemini LLMProvider._generate
- WechatServiceImplProfileTest
- docs/docker/start.sh
- OpenAIStyleLLMServiceImpl
- WebSocketClientManager
- eslint-plugin-format
- Result.java
- postcss-html
- AgentSnapshotService
- @uni-helper/vite-plugin-uni-components
- @uni-helper/vite-plugin-uni-pages
- @uni-helper/vite-plugin-uni-platform
- unocss
- @unocss/preset-legacy-compat
- unplugin-auto-import
- vite-plugin-restart
- vue-tsc
- default.vue
- .current
- NfcWriteJobDialog.vue
- AccessToken
- Frontend App Mount
- DeepSeek Charset
- GB2312 Charset
- main script
- AliBL LLMProvider.response
- FastGPT LLMProvider.response
- HomeAssistant LLMProvider.response
- Ollama LLMProvider.response
- Xinference LLMProvider.response
- MemLocalShort MemoryProvider.init_memory
- MemReportOnly MemoryProvider.query_memory
- MemReportOnly MemoryProvider.save_memory
- UnifiedToolHandler._combine_responses
- IndexStream TTSProvider.text_to_speak
- MiniMax HTTP Stream TTSProvider.text_to_speak
- PaddleSpeech TTSProvider.pcm_to_wav
- Xunfei Stream TTSProvider.text_to_speak
- manager cache_manager
- xiaozhi-server/start.sh
- privacy policy page controller
- settings page controller
- 小智后台管理系统（Manager API）
- run-build-manager.sh
- run-build-web.sh
- run-build-xiaozhi.sh
- run-mirror-funasr.sh
- NfcManualWrite.vue
- wechat avatar upload endpoint
- NfcActivationManagement.vue
- PdcNfcLiquibaseIntegrationTest.java
- ChunkMethod
- BCrypt
- Prototype
- da
- AgentTemplateService
- AgentChatAudioService
- AgentChatHistoryBizService
- AgentChatHistoryService
- AgentChatSummaryService
- getAudioPlayUrl
- AgentChatTitleService
- AgentContextProviderService
- button Component
- egg-avatar Component
- mood-badge Component
- nav-bar Component
- pet-avatar Component
- convert_percentage_to_range
- AgentEntity
- AgentMcpAccessPointService
- AgentPluginMappingService
- AgentService
- CustomTabs
- AgentSnapshotService
- AgentTagService
- AgentTemplateEntity
- AgentTemplateService
- AgentVoicePrintService
- ASRProviderBase
- BaseEntity
- Env Types
- BaseHandler
- ESLint Config
- CaptchaService
- CollectionCardImageService
- CompanionCreateDTO
- CompanionService
- ConfigService
- CorrectWordFileService
- DeviceAddressBookService
- DeviceService
- Exception
- FulfillmentDispatcher
- HatchActionService
- IntentProviderBase
- InviteService
- ItemFulfillmentService
- vuex
- ItemService
- KnowledgeBaseAdapter
- KnowledgeBaseService
- KnowledgeFilesService
- KnowledgeManagerService
- LLMProviderBase
- LLMService
- LLMProviderBase.response_with_functions
- MemoryProviderBase
- ModelBasicInfoDTO
- ModelConfigService
- ModelProviderService
- OtaService
- PasswordEncoder
- PetCollectionCardService
- PetService
- Plugin
- Protocol
- RuntimeException
- ShiroService
- SmartInitializingSingleton
- SmsService
- SubscriptionFulfillmentService
- SubscriptionService
- SysDictDataService
- SysDictTypeService
- SysUserService
- SysUserTokenService
- SysUserUtilService
- TextMessageHandler
- TimbreService
- TokenService
- ToolExecutor
- TTSProviderBase
- VADProviderBase
- VLLMProviderBase
- VoiceCloneService
- WechatService
- iE
- NfcStockInDialog.vue
- localizedSnapshotDisplayValue
- PdcNfcBatchAdminController.java
- nfc-claim-api.test.js
- pet-api.test.js
- NfcWriteResultImportDialog.vue
- .toStringObjectMap
- pages/privacy/privacy
- com.fasterxml.jackson.databind.ObjectMapper
- PdcNfcManualMarkAction
- NfcBatchDialog.vue
- KnowledgeBaseDTO
- knowledgeBase.js
- RAGFlowClient
- ChunkDTO
- FieldMetaObjectHandler.java
- PdcNfcControllerResultContractTest.java
- AgentTemplateServiceImpl
- PdcNfcManualWriteServiceImpl
- PdcNfcPrototype.java
- KnowledgeFilesService
- TokenServiceImpl.java
- DeviceOtaVO
- ImmediateFuture
- cross-env
- PdcNfcClaimPreviewDTO.java
- opus-decoder
- opus-recorder
- vue-axios
- pdcNfcApi.test.mjs
- ASRProvider
- TTSPerformanceTester
- soul-resonance.js
- .enabledAutoUpdateChecksFirmware
- DialogueRecorder
- PdcNfcProductTypeAdminController
- handleDrag
- .__init__
- DeviceManualAddDTO
- AiAgentChatAudioDao.getOssKeysByAudioIds
- AiAgentChatAudioDao.insert
- AiAgentChatAudioDao.selectById
- AiAgentChatAudioDao.updateById
- AiAgentChatHistoryDao.deleteAudioByIds
- AiAgentChatHistoryDao.getAudioIdsByAgentId
- CompanionDao.insert
- CompanionDao.selectById
- CompanionDao.selectOne
- CompanionDao.selectPage
- CompanionDao.updateById
- DeviceService.getAgentIdByDeviceId
- ai_companion menstrual cycle migration
- Post Upgrade Script
- clear
- setAllFunctions
- setCurrentAgentId
- setCurrentFunctions
- updateFunctions
- updateProviders
- resetChangedFields
- setUserAvatar
- VoiceButton Component
- voice-catalog config
- backpack logic
- Change Soul onQuirkTap
- Change Soul onTraitTap
- refreshLogin
- Tool Use Prompt Format
- MemLocalShort MemoryProvider.query_memory
- VolMem0 MemoryProvider.query_memory
- UnifiedToolHandler.cleanup
- UnifiedToolHandler.get_functions
- Coze TTSProvider.text_to_speak
- Custom TTSProvider.text_to_speak
- DefaultTTS.text_to_speak
- Doubao TTSProvider.text_to_speak
- GPT SoVITS V2 TTSProvider.text_to_speak
- GPT SoVITS V3 TTSProvider.text_to_speak
- OpenAI TTSProvider.text_to_speak
- SiliconFlow TTSProvider.text_to_speak
- Tencent TTSProvider.text_to_speak
- Xunfei Stream TTSProvider._start_monitor_tts_response
- demo model
- Manifest Config
- MemoryAnchor Page
- OpusEncoderUtils.__init__
- OpusEncoderUtils.close
- OpusEncoderUtils.reset_state
- xiaozhi:xiaozhi-esp32-api
- 小智服务器项目文档中心
- Global Types
- VisionHandler._create_error_response
- voiceprint_provider identify_speaker
- voiceprint_provider VoiceprintProvider

## God Nodes (most connected - your core abstractions)
1. `Result` - 329 edges
2. `getServiceUrl()` - 193 edges
3. `PageData` - 178 edges
4. `AgentSnapshotServiceImpl` - 170 edges
5. `RenException` - 141 edges
6. `ConnectionHandler` - 118 edges
7. `ErrorCode` - 110 edges
8. `setup_logging()` - 110 edges
9. `t()` - 104 edges
10. `error()` - 97 edges

## Surprising Connections (you probably didn't know these)
- `settings Page` --references--> `Bond Panel (羁绊面板)`  [EXTRACTED]
  main/miniprogram/pages/settings/settings.js → main/miniprogram/pages/settings/settings.wxml
- `settings Page` --references--> `Letter Popup (一封写给你的信)`  [EXTRACTED]
  main/miniprogram/pages/settings/settings.js → main/miniprogram/pages/settings/settings.wxml
- `manager-api` --conceptually_related_to--> `OTA WebSocket 视觉分析配置`  [INFERRED]
  docs/architecture/xiaozhi-esp32-server-architecture.html → main/digital-human/index.html
- `load_config()` --semantically_similar_to--> `TestRuntimeHttpServer`  [INFERRED] [semantically similar]
  main/digital-human/wakeword_runtime/config/config_loader.py → main/digital-human/wakeword_runtime/runtime/http_server.py
- `DetectorAssetsBuilder` --semantically_similar_to--> `TestRuntimeHttpServer`  [INFERRED] [semantically similar]
  main/digital-human/wakeword_runtime/core/detector_assets.py → main/digital-human/wakeword_runtime/runtime/http_server.py

## Import Cycles
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/addressBook.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/voiceResource.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/correctWord.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/admin.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/agent.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/device.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/dict.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/feedback.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/knowledgeBase.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/model.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/ota.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/pdcNfc.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/storyEngine.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/timbre.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/user.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`
- 4-file cycle: `main/manager-web/src/apis/api.js -> main/manager-web/src/apis/module/voiceClone.js -> main/manager-web/src/apis/httpRequest.js -> main/manager-web/src/store/index.js -> main/manager-web/src/apis/api.js`

## Hyperedges (group relationships)
- **Manager API Domain Modules** — qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_智能体_agent_管理模块_module_agent_module, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_ai伴侣管理模块_module_companion_module, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_ai宠物孵化与陪伴模块_module_pet_module, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_设备管理与ota升级模块_module_device_module, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_知识库与rag文档管理模块_module_knowledge_module, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_模型配置管理与llm服务调用_module_model_llm, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_支付与订阅管理模块_module_payment_subscription, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_声音克隆与音色管理模块_module_voice_features, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_微信小程序集成模块_module_wechat_integration [EXTRACTED 1.00]
- **Manager Admin Frontends** — qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_module_manager_api, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智移动端管理后台_uniapp_多端应用_module_manager_mobile, qoder_repowiki_knowledge_zh_index_manager_web [EXTRACTED 1.00]
- **Manager API Supporting Infrastructure** — qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_通用框架基础组件_module_common_framework, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_manager_api_运行时资源与数据库迁移配置_module_manager_api_resources_config, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_安全认证与授权模块_module_security_auth, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_系统管理模块_用户_字典_参数与服务器管理_module_sys_admin, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智后台管理系统_manager_api_manager_api_单元测试套件_module_manager_api_tests [INFERRED 0.85]
- **Core Provider Adapter Layer** — qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_核心能力提供商适配层_asr_tts_llm_memory_intent_vad_vllm_module_providers, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_核心能力提供商适配层_asr_tts_llm_memory_intent_vad_vllm_语音识别_asr_提供商适配层_module_providers_asr, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_核心能力提供商适配层_asr_tts_llm_memory_intent_vad_vllm_tts语音合成服务提供者集合_module_providers_tts, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_核心能力提供商适配层_asr_tts_llm_memory_intent_vad_vllm_大语言模型_llm_提供商适配层_module_providers_llm, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_核心能力提供商适配层_asr_tts_llm_memory_intent_vad_vllm_记忆存储提供者_memory_providers_module_providers_memory, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_核心能力提供商适配层_asr_tts_llm_memory_intent_vad_vllm_意图识别提供者_intent_provider_module_providers_intent, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_核心能力提供商适配层_asr_tts_llm_memory_intent_vad_vllm_统一工具执行框架_多源工具路由与执行_module_providers_tools [INFERRED 0.95]
- **Agent Message Tool Plugin Flow** — qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_agent_编排核心_消息处理与插件初始化_module_agent_orchestration, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_agent_编排核心_消息处理与插件初始化_文本消息处理器与路由分发_module_message_handlers, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_核心能力提供商适配层_asr_tts_llm_memory_intent_vad_vllm_统一工具执行框架_多源工具路由与执行_module_providers_tools, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_插件函数系统与工具注册中心_module_plugin_system [INFERRED 0.85]
- **Client Device Server Connectivity** — qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_http_api_服务与设备接口处理器_module_http_api, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音交互服务器_xiaozhi_server_websocket_连接服务与认证管理_module_websocket_connection, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_小智语音助手微信小程序前端_module_miniprogram, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_蛋宝宝微信小程序前端_egg_baby_miniprogram_module_egg_miniprogram, qoder_repowiki_knowledge_zh_小智_esp32_语音交互系统_xiaozhi_esp32_server_数字人本地测试与唤醒词运行时_module_digital_human [INFERRED 0.85]
- **Voice AI Pipeline** — docs_architecture_xiaozhi_esp32_server_architecture_xiaozhi_server, docs_architecture_xiaozhi_esp32_server_architecture_voice_ai_pipeline, docs_architecture_xiaozhi_esp32_server_architecture_provider_pattern [EXTRACTED 1.00]
- **Payment Consistency Defenses** — docs_diagrams_payment_flow_improved_wechat_callback_defense, docs_diagrams_payment_flow_improved_frontend_active_query, docs_diagrams_payment_flow_improved_scheduled_task_compensation [EXTRACTED 1.00]
- **Privacy Sensitive AI Data** — main_manager_web_public_privacy_policy_personal_information_processing, main_manager_web_public_privacy_policy_ai_interaction_data, main_manager_web_public_privacy_policy_third_party_ai_services [EXTRACTED 1.00]
- **Generator Static Resources** — main_manager_web_public_generator_static_charsets_deepseek_charset, main_manager_web_public_generator_static_charsets_gb2312_charset, main_manager_web_public_generator_static_charsets_latin1_charset, main_manager_web_public_generator_static_charsets_qwen18409_charset, main_manager_web_public_generator_static_multinet_model_fst_commands_cn_chinese_fst_commands, main_manager_web_public_generator_static_multinet_model_fst_commands_en_english_fst_commands [INFERRED 0.85]
- **Default Selected Modules** — main_xiaozhi_server_config_selected_module, main_xiaozhi_server_config_funasr, main_xiaozhi_server_config_chatglm_llm, main_xiaozhi_server_config_edge_tts, main_xiaozhi_server_config_nomem, main_xiaozhi_server_config_plugins [EXTRACTED 1.00]
- **Three Level Memory Architecture** — main_xiaozhi_server_docs_architecture_memory_system_architecture_semantic_memory, main_xiaozhi_server_docs_architecture_memory_system_architecture_episodic_memory, main_xiaozhi_server_docs_architecture_memory_system_architecture_user_profile_memory [EXTRACTED 1.00]

## Communities (641 total, 226 thin omitted)

### Community 0 - "Result"
Cohesion: 0.04
Nodes (20): io.swagger.v3.oas.annotations.Operation, io.swagger.v3.oas.annotations.Parameter, Result, AgentController, AgentChatSummaryService, CompanionController, GetMapping, PostMapping (+12 more)

### Community 1 - "index-Guo1hQ-y.js"
Cohesion: 0.01
Nodes (237): _0, A0, Ab(), Ad, Ag(), Ah, Al(), Am (+229 more)

### Community 2 - "SysUserTokenEntity"
Cohesion: 0.07
Nodes (19): RenExceptionHandler, CharacterAge, SysUserTokenDao, SysUserTokenEntity, Override, Oauth2Realm, Override, ShiroServiceImpl (+11 more)

### Community 3 - "BaseService"
Cohesion: 0.04
Nodes (21): TokenDTO, BaseService, Override, SysUserTokenServiceImpl, SysUserTokenService, PasswordDTO, SysUserDTO, Override (+13 more)

### Community 4 - "AgentSnapshotServiceImplTest.java"
Cohesion: 0.10
Nodes (8): AgentTagDao, AgentTagRelationDao, AgentTagDTO, AgentTagEntity, AgentTagRelationEntity, AgentTagService, AgentTagServiceImpl, Override

### Community 5 - "getServiceUrl"
Cohesion: 0.02
Nodes (170): getServiceUrl(), getAddressBookList(), updateAlias(), updatePermission(), addParam(), deleteParam(), deleteUser(), getParamsList() (+162 more)

### Community 6 - "ActionResponse"
Cohesion: 0.03
Nodes (68): send_display_message(), ABC, Any, ToolExecutor, Enum, ToolDefinition, ToolType, DeviceIoTExecutor (+60 more)

### Community 7 - "TTSProvider"
Cohesion: 0.11
Nodes (10): Header, Optional, Any, 建立新的WebSocket连接，并启动监听任务（仅第一次）, 发送 FinishConnection 事件，等待服务端返回 EVENT_ConnectionFinished, 重写父类方法：使用独立的临时编码器处理音频文件，避免与TTS流式编码器并发冲突。 双流式TTS中，monitor任务在event…, 非流式生成音频数据，用于生成音频及测试场景 Args: text: 要转换的文本 Returns: list: 音频数据列表, Response (+2 more)

### Community 8 - "lombok.Data"
Cohesion: 0.03
Nodes (62): io.swagger.v3.oas.annotations.media.Schema, lombok.Data, JsonRpcTwo, TreeNode, AgentChatSummaryDTO, AgentMemoryDTO, AgentSnapshotRestoreDTO, AgentSnapshotTagDTO (+54 more)

### Community 9 - "CompanionServiceImplTest"
Cohesion: 0.13
Nodes (3): CompanionService, CompanionServiceImplTest, CompanionMoodRefreshTaskTest

### Community 10 - "StoryEngineController.java"
Cohesion: 0.08
Nodes (13): BatchWeightUpdateDTO, SmallSceneWeightItem, BigSceneDTO, SmallSceneDTO, Override, StoryBigSceneServiceImpl, Override, StorySmallSceneServiceImpl (+5 more)

### Community 11 - "AsyncConfig.java"
Cohesion: 0.60
Nodes (3): AsyncConfig, org.springframework.context.annotation.EnableAspectJAutoProxy, org.springframework.scheduling.annotation.EnableAsync

### Community 12 - "AgentSnapshotPanel.vue"
Cohesion: 0.04
Nodes (105): deleteAgentSnapshot(), getAgentSnapshot(), getAgentSnapshots(), getCorrectWordFiles(), getTTSVoices(), restoreAgentSnapshot(), AgentSnapshotData, attachPreviousRows() (+97 more)

### Community 13 - "ConnectionHandler"
Cohesion: 0.03
Nodes (38): ThreadPoolExecutor, build_module_string(), create_connection_logger(), get_module_abbreviation(), 为连接创建独立的日志器，绑定特定的模块字符串, 获取模块名称的缩写，如果为空则返回00 如果名称中包含下划线，则返回下划线后面的前两个字符, ConnectionHandler, Any (+30 more)

### Community 14 - "KnowledgeBaseServiceImpl"
Cohesion: 0.19
Nodes (3): Override, SuppressWarnings, KnowledgeBaseServiceImpl

### Community 15 - "PdcNfcJobStateMachineTest"
Cohesion: 0.11
Nodes (9): PdcNfcSchemeJobStatus, CANCELLED, FAILED, PARTIAL_SUCCESS, PENDING, RUNNING, SUCCEEDED, PdcNfcSchemeJobStateMachine (+1 more)

### Community 16 - "aA"
Cohesion: 0.06
Nodes (46): aA(), Au(), BE(), Bg(), bo(), Bs(), $c(), Cg() (+38 more)

### Community 17 - "StoryEvaluationResult"
Cohesion: 0.09
Nodes (16): StoryRuntimeConfig, StoryPetPrototype, KOI, RABBIT, StoryEvaluationResult, INITIALIZED, KEPT_INVALID_CONFIGURATION, KEPT_NOT_DUE (+8 more)

### Community 18 - "httpRequest.js"
Cohesion: 0.05
Nodes (43): fly, httpHandlerError(), reAjaxFun(), sendRequest(), changeLanguage(), i18n, register(), router (+35 more)

### Community 19 - "connection.py"
Cohesion: 0.04
Nodes (54): ensure_directories(), get_config_from_api_async(), get_private_config_from_api(), get_project_dir(), load_config(), merge_configs(), 递归合并配置，custom_config优先级更高 Args: default_config: 默认配置 custom_config: 用户自定义配置…, read_config() (+46 more)

### Community 20 - "org.junit.jupiter.api.DisplayName"
Cohesion: 0.03
Nodes (15): ImageUploadServiceTest, PdcNfcAdminPermissionContractTest, PdcNfcProductTypeAdminControllerTest, ErrorCodeMessages, MetricsComponent, PdcNfcBackendAcceptanceTest, PdcNfcControllerPermissionContractTest, PdcNfcSensitiveLoggingContractTest (+7 more)

### Community 21 - "edit.vue"
Cohesion: 0.03
Nodes (104): getAgentDetail(), getAgentTags(), getAllLanguage(), getModelOptions(), getPluginFunctions(), getRoleTemplates(), updateAgent(), Agent (+96 more)

### Community 22 - "AgentChatHistoryBizServiceImpl"
Cohesion: 0.26
Nodes (4): AgentChatHistoryReportDTO, AgentChatHistoryBizService, AgentChatHistoryBizServiceImpl, Override

### Community 23 - ".push"
Cohesion: 0.03
Nodes (58): Ba(), Bd, bt(), Cd, Ck(), cs(), dv(), ev() (+50 more)

### Community 25 - "t"
Cohesion: 0.05
Nodes (46): createAgent(), deleteAgent(), getAgentList(), t(), checkESP32Connection(), checkingConnection, emit, getSignalStrength() (+38 more)

### Community 26 - "lombok.RequiredArgsConstructor"
Cohesion: 0.03
Nodes (90): java.util.concurrent.ThreadPoolExecutor, javax.sql.DataSource, lombok.RequiredArgsConstructor, PropertiesUtils, CompanionMoodRefreshTask, DocumentStatusSyncTask, PdcNfcAssetStatus, ACTIVE (+82 more)

### Community 27 - "nQ"
Cohesion: 0.04
Nodes (96): AC(), aI(), AQ(), _B(), Bc(), Bn(), ce(), CQ() (+88 more)

### Community 28 - "settings/index.vue"
Cohesion: 0.09
Nodes (31): getSupportedLanguages(), baseUrlInput, cacheInfo, clearAllCacheAfterUrlChange(), clearCache(), configStore, currentLanguage, getCacheInfo() (+23 more)

### Community 29 - "xiaozhi-server/app.py"
Cohesion: 0.06
Nodes (32): main(), monitor_stdin(), 阻塞直到收到 Ctrl‑C / SIGTERM。 - Unix: 使用 add_signal_handler - Windows: 依赖…, wait_for_exit(), BaseHandler, 处理OPTIONS请求，添加CORS头信息, _is_higher_version(), OTAHandler (+24 more)

### Community 30 - "kl"
Cohesion: 0.06
Nodes (4): kl(), ns, os(), sI()

### Community 31 - "pet-store.js"
Cohesion: 0.04
Nodes (65): onLogout(), petStore, onValidate(), onShow(), petStore, onShow(), petApi, petStore (+57 more)

### Community 32 - "lombok.AllArgsConstructor"
Cohesion: 0.13
Nodes (29): io.swagger.v3.oas.annotations.Parameters, io.swagger.v3.oas.annotations.tags.Tag, lombok.AllArgsConstructor, ImageUploadController, ValidatorUtils, AgentMcpAccessPointController, AgentVoicePrintController, CompanionContextController (+21 more)

### Community 33 - "MCPClient"
Cohesion: 0.04
Nodes (31): MCPClient, any, Exception, Future, 设备端MCP客户端，用于管理MCP状态和工具, call_mcp_tool(), call_mcp_tool, handle_mcp_message() (+23 more)

### Community 34 - "egg-miniprogram/miniprogram/app.js"
Cohesion: 0.05
Nodes (62): applySession(), auth, AUTH_FIELDS, { captureNfcClaimIntent }, clearLoginState(), ensureLogin(), loginWithWechat(), onLaunch() (+54 more)

### Community 35 - "index/index.js"
Cohesion: 0.05
Nodes (60): DELETE /agent/chat-history/{id} API, GET /agent/chat-history/list API, _addMessage(), app, _appendStreamingText(), _applyFeatures(), AudioManager, _bootstrap() (+52 more)

### Community 36 - ".get"
Cohesion: 0.08
Nodes (11): io.swagger.v3.oas.annotations.Hidden, jakarta.servlet.http.HttpServletResponse, lombok.SneakyThrows, AgentChatHistoryController, OTAController, Override, QueryWrapper, SysDictDataServiceImpl (+3 more)

### Community 37 - "TTSProviderBase"
Cohesion: 0.03
Nodes (35): BaseModel, TTSProvider, ABC, Any, 存储指定 sentence_id 对应的文本，用于流式TTS获取正确的字幕文本 Args: sentence_id: 会话ID text: 要存储的文本, 获取指定 sentence_id 对应的文本 Args: sentence_id: 会话ID Returns: str: 对应的文本，如果不存在返回 None, 清除指定 sentence_id 的文本 Args: sentence_id: 会话ID, 处理音频文件并转换为指定格式 Args: tts_file: 音频文件路径 callback: 文件处理函数 (+27 more)

### Community 38 - "PetServiceImplTodayMoodTest"
Cohesion: 0.06
Nodes (7): BirthResult, Override, MbtiParser, BirthResult, PetBirthCalculator, PetServiceImplTodayMoodTest, PetBirthCalculatorTest

### Community 39 - "TextMessageType"
Cohesion: 0.07
Nodes (27): handleHelloMessage(), handleTextMessage(), AbortTextMessageHandler, Any, HelloTextMessageHandler, Any, IotTextMessageHandler, Any (+19 more)

### Community 40 - "PdcNfcProperties"
Cohesion: 0.03
Nodes (40): java.util.regex.Pattern, DateConverter, Override, SM2Utils, ClaimRef, PdcNfcProperties, ClaimRefProtection, EncryptedField (+32 more)

### Community 41 - "test_chat_integration.py"
Cohesion: 0.04
Nodes (47): Enum, fixture, _async_false(), _async_none(), chat_connection(), connection_module(), crisis_input_connection(), direct_answer_connection() (+39 more)

### Community 42 - "devDependencies"
Cohesion: 0.03
Nodes (71): @alova/wormhole, @antfu/eslint-config, autoprefixer, @commitlint/cli, @commitlint/config-conventional, @dcloudio/types, @dcloudio/uni-automator, @dcloudio/uni-cli-shared (+63 more)

### Community 43 - "change-soul.js"
Cohesion: 0.06
Nodes (38): getLabel(), companion-codes config, OCCUPATIONS, QUIRKS, ROLES, SOUL_TRAITS, codes, { get, post } (+30 more)

### Community 44 - "doodle-api.js"
Cohesion: 0.07
Nodes (21): API_BASE_URL, { API_BASE_URL }, auth, { post, get }, assert, doodleApi, Module, requestResponses (+13 more)

### Community 45 - "register/index.vue"
Cohesion: 0.04
Nodes (51): CaptchaResponse, ForgotPasswordData, getCaptcha(), getPublicConfig(), LoginData, LoginResponse, PublicConfig, register() (+43 more)

### Community 46 - "home.js"
Cohesion: 0.05
Nodes (74): auth, buildCompanionActions(), buildShareQuery(), CHAT_ENTRY_ICONS, chatEntryIcon(), clearEnvironmentTimer(), clearStoryCaptionRotation(), clearStoryCaptionToast() (+66 more)

### Community 47 - "InviteServiceImplTest"
Cohesion: 0.10
Nodes (9): InviteCodeDao, InviteCodeUpdateDTO, InviteCodeEntity, InviteServiceImpl, Override, PageData, InviteCodeVO, InviteConsumeConcurrencyTest (+1 more)

### Community 48 - "AgentChatHistoryService"
Cohesion: 0.06
Nodes (29): jakarta.servlet.http.HttpServletRequest, OperationLogAspect, RedisAspect, HttpContextUtils, IpUtils, AgentChatTitleDao, AgentDTO, AgentChatHistoryEntity (+21 more)

### Community 49 - "PdcNfcInventoryServiceImpl"
Cohesion: 0.07
Nodes (19): GetMapping, PostMapping, RequestMapping, RestController, PdcNfcAssetAdminController, GetMapping, RequestMapping, RestController (+11 more)

### Community 50 - ".generate"
Cohesion: 0.07
Nodes (12): LeaseLostException, WechatNfcErrorAction, QUOTA_DEFER, RETRYABLE, TASK_FATAL, WechatNfcErrorPolicy, JumpWxa, WechatNfcSchemeRequest (+4 more)

### Community 51 - "tools.vue"
Cohesion: 0.07
Nodes (25): getMcpAddress(), getMcpTools(), agentId, allFunctions, arrayTextCache, copyMcpAddress(), currentFunction, currentSegmented (+17 more)

### Community 52 - "get"
Cohesion: 0.10
Nodes (25): GET /companion/detail API, GET /companion/intimacy API, GET /item/inventory API, all(), DEFAULT_VOICES, findById(), assert, cat (+17 more)

### Community 53 - "get"
Cohesion: 0.05
Nodes (48): FALLBACK_AGE_RANGES, API_BASE_URL, ageRangeApi, { FALLBACK_AGE_RANGES }, loadAgeRange(), loadOptions(), normalizeAgeRange(), onConfirm() (+40 more)

### Community 54 - "Alova HTTP Client"
Cohesion: 0.06
Nodes (50): getAudioId, getChatHistory, getChatSessions, createVoicePrint(), getChatHistoryUser(), getVoicePrintList(), updateAgentTags(), ApiResponse (+42 more)

### Community 55 - "DeviceServiceImpl"
Cohesion: 0.10
Nodes (5): DeviceServiceImpl, Activation, Firmware, Override, ServerTime

### Community 56 - "doodle-editor.js"
Cohesion: 0.07
Nodes (47): beginPinch(), cacheBrushSizeTrack(), canvas, canvasPoint(), clearCanvasNoticeTimers(), collapseToolPanel(), currentBrush(), detached() (+39 more)

### Community 57 - "dependencies"
Cohesion: 0.03
Nodes (59): abortcontroller-polyfill, alova, @alova/adapter-uniapp, @alova/shared, dayjs, @dcloudio/uni-app, @dcloudio/uni-app-harmony, @dcloudio/uni-app-plus (+51 more)

### Community 58 - "ASRProvider"
Cohesion: 0.07
Nodes (11): ASRProvider, ASRPerformanceTester, main(), 从 data 目录加载所有 .config.yaml 文件的配置, ASRPerformanceSuite, BaseASRTester, DoubaoStreamASRTester, main() (+3 more)

### Community 59 - "J"
Cohesion: 0.10
Nodes (21): Af, Bf, Cf, _d, dd, df, el(), fd (+13 more)

### Community 60 - "MemoryProviderBase"
Cohesion: 0.04
Nodes (26): generate_and_save_chat_summary(), LLMProviderBase.response_no_stream, MemoryProviderBase, ABC, Save a new memory for specific role and return memory ID, Query memories for specific role based on similarity, MemoryProvider, extract_json_data() (+18 more)

### Community 61 - "is"
Cohesion: 0.08
Nodes (3): es, is(), Ts()

### Community 62 - "PaymentOrderEntity"
Cohesion: 0.04
Nodes (36): cn.hutool.json.JSONObject, JSONObject, SensitiveDataUtils, ItemGrantSource, ItemFulfillmentServiceImpl, JSONObject, Override, ItemFulfillmentService (+28 more)

### Community 64 - "device/index.vue"
Cohesion: 0.08
Nodes (35): bindDevice(), bindDeviceManual(), getBindDevices(), getFirmwareTypes(), unbindDevice(), updateDeviceAutoUpdate(), Device, FirmwareType (+27 more)

### Community 65 - "PdcNfcWriteResultImporterTest"
Cohesion: 0.10
Nodes (4): Override, PdcNfcWriteResultImporterImpl, PdcNfcWriteResultImporterTest, org.springframework.mock.web.MockMultipartFile

### Community 66 - "roleConfig.vue"
Cohesion: 0.07
Nodes (36): applyTemplateData(), applyTtsDraft(), buildCurrentFunctions(), buildTtsDraft(), captureTtsDraft(), cloneTtsDraft(), enrichCurrentFunctionsWithMetadata(), fetchAgentConfig() (+28 more)

### Community 67 - "voice-call.js"
Cohesion: 0.11
Nodes (27): app, AudioManager, _cleanup(), _cleanupResources(), _computeStatusText(), formatDuration(), { getTheme, applyTheme }, _handleWSMessage() (+19 more)

### Community 68 - "MoodDeciderTest"
Cohesion: 0.09
Nodes (10): MoodActingGuide, fromLabel(), TodayMood, CALM, EXCITED, HAPPY, LOW, MISS (+2 more)

### Community 69 - "org.springframework.web.socket.WebSocketSession"
Cohesion: 0.15
Nodes (12): InternalHandler, Override, Override, WebSocketTestHandler, org.springframework.util.StopWatch, org.springframework.web.socket.BinaryMessage, org.springframework.web.socket.CloseStatus, org.springframework.web.socket.handler.AbstractWebSocketHandler (+4 more)

### Community 70 - "AgentSnapshotDialog.vue"
Cohesion: 0.08
Nodes (31): beforeDestroy(), cancelPendingSnapshotRequests(), canRestoreSnapshot(), canViewSnapshot(), CHAT_HISTORY_CONF_LABEL_KEYS, close(), detailDialogTitle(), ensureCorrectWordMetadata() (+23 more)

### Community 71 - "org.apache.ibatis.annotations.Mapper"
Cohesion: 0.06
Nodes (28): com.baomidou.mybatisplus.core.mapper.BaseMapper, BaseDao, AgentPluginMappingMapper, AgentTemplateDao, CompanionDao, FeedbackDao, InviteUsageDao, ItemConsumeLogDao (+20 more)

### Community 72 - "scripts"
Cohesion: 0.04
Nodes (51): scripts, build, build:app, build:app-android, build:app-ios, build:custom, build:h5, build:h5:ssr (+43 more)

### Community 73 - "utils/index.ts"
Cohesion: 0.05
Nodes (48): getUserInfo(), UserInfo, usePageAuth, isLogined(), usePageAuth(), TFile, TfileType, TImage (+40 more)

### Community 74 - "CorrectWordFileService"
Cohesion: 0.09
Nodes (13): AgentCorrectWordMappingDao, AgentCorrectWordMappingEntity, CorrectWordFileDao, CorrectWordItemDao, CorrectWordFileCreateDTO, CorrectWordFileEntity, CorrectWordItemEntity, CorrectWordFileService (+5 more)

### Community 76 - "TestRuntimeApplication"
Cohesion: 0.10
Nodes (6): AudioPlugin, Exception, WakeWordPlugin, EventPublisher, Protocol, TestRuntimeApplication

### Community 77 - "i"
Cohesion: 0.03
Nodes (73): ae(), Ak(), Av(), bI(), Bk(), Bu, Bv(), cA() (+65 more)

### Community 78 - "detail.vue"
Cohesion: 0.06
Nodes (35): getAudioId(), getChatHistory(), getChatSessions(), AudioResponse, ChatMessage, ChatSession, ChatSessionsResponse, GetSessionsParams (+27 more)

### Community 79 - ".sourceToTarget"
Cohesion: 0.05
Nodes (15): ModelController, ModelProviderController, ModelConfigDao, ModelBasicInfoDTO, ModelConfigBodyDTO, ModelConfigDTO, ModelProviderDTO, ModelConfigEntity (+7 more)

### Community 80 - "AgentSnapshotServiceImpl"
Cohesion: 0.08
Nodes (7): AgentSnapshotServiceImpl, SuppressWarnings, SensitivePathAnalysis, SensitivePathSlot, SensitiveUrlParts, UrlParameter, UrlPathParts

### Community 81 - "ProviderManagement.vue"
Cohesion: 0.05
Nodes (19): created(), fetchFeedbackList(), fetchTypeOptions(), goToPage(), handlePageSizeChange(), handleSave(), handleSearch(), created() (+11 more)

### Community 82 - "manager-api"
Cohesion: 0.05
Nodes (45): ESP32 设备, manager-api, manager-web 与 manager-mobile, MySQL Redis OceanBase PowerMem, Provider 模式, OAuth2 服务密钥 XSS SQL 注入 HTTPS, 小智ESP32服务器系统架构, ASR LLM TTS 语音流水线 (+37 more)

### Community 83 - "ContentSafetyGate"
Cohesion: 0.15
Nodes (44): ContentSafetyContext, ContentSafetyGate, OutputSafetyGate, allowed(), blocked(), config(), crisis_blocked(), crisis_watched() (+36 more)

### Community 84 - "utils_vad.py"
Cohesion: 0.12
Nodes (19): Silero Voice Activity Detector Returns a model with a set of utils Please see…, silero_vad(), versiontuple(), load_silero_vad(), collect_chunks(), drop_chunks(), get_speech_timestamps(), init_jit_model() (+11 more)

### Community 85 - "error"
Cohesion: 0.18
Nodes (14): checkServiceWorkerStatus(), isMobileDevice(), mounted(), clearCache(), loadCacheData(), refreshCache(), visible(), checkCdnCacheStatus() (+6 more)

### Community 86 - "home.vue"
Cohesion: 0.06
Nodes (12): clearSearchHistory(), confirmDeleteAgent(), fetchAgentList(), handleSearch(), handleSearchReset(), handleWisdomBodyAdded(), loadFeatureStatus(), loadSearchHistory() (+4 more)

### Community 87 - "pet-store.test.js"
Cohesion: 0.05
Nodes (37): accountKeys, assert, cardsPet, cardsVO, coldHatched, coldHatchedVO, daily, dateKey() (+29 more)

### Community 90 - "main/miniprogram/app.js"
Cohesion: 0.13
Nodes (20): { applyGlobalTheme }, checkDeviceStatus(), { checkOrRegisterDevice, completeDeviceBinding }, clearLoginState(), ensureAgentExists(), fetchCompanionData(), fetchSubscription(), initInBackground() (+12 more)

### Community 91 - "Constant.java"
Cohesion: 0.06
Nodes (33): ChatHistoryConfEnum, IGNORE, RECORD_TEXT, RECORD_TEXT_AUDIO, DataOperation, DELETE, INSERT, UPDATE (+25 more)

### Community 92 - "MicrophoneListener"
Cohesion: 0.18
Nodes (5): AudioListener, MicrophoneListener, ndarray, Protocol, Any

### Community 93 - "chat.js"
Cohesion: 0.07
Nodes (49): _addMessage(), ageRangeApi, _appendStreamingText(), _appendWithSeparator(), AudioManager, _checkDailyChatLimit(), CONN_LABELS, _finalizeStreaming() (+41 more)

### Community 94 - "collection-card.js"
Cohesion: 0.07
Nodes (29): birthdayLabel(), _drawPetAvatar(), drawShareCard(), _ellipse(), genderClass(), genderLabel(), inviteApi, isUsableShareInvite() (+21 more)

### Community 95 - "PromptManager"
Cohesion: 0.06
Nodes (29): AudioRateController, 音频速率控制器 - 按照60ms帧时长精确控制音频发送 解决高并发下的时间累积误差问题, 启动异步发送任务 Args: send_audio_callback: 发送音频的回调函数 Returns: asyncio.Task: 发送任务, Args: frame_duration: 单个音频帧时长（毫秒），默认60ms, 检查队列并按时发送音频/消息 Args: send_audio_callback: 发送音频的回调函数 async def(opus_packet), ContextDataProvider, Any, 获取 manager-api 基址与服务密钥，用于解析内部相对上下文源。 优先取本地合并配置的 manager-api 段，缺失时回退到已初始化的… (+21 more)

### Community 96 - "org.springframework.transaction.annotation.Transactional"
Cohesion: 0.04
Nodes (33): com.baomidou.mybatisplus.core.conditions.Wrapper, com.baomidou.mybatisplus.core.enums.SqlMethod, Override, SuppressWarnings, DocumentDao, DocumentEntity, BatchIdReq, InfoVO (+25 more)

### Community 97 - "ultrasonic-config.vue"
Cohesion: 0.11
Nodes (24): arrayBufferToBase64(), audioContext, audioFilePath, audioGenerated, audioLengthText, autoLoop, base64Encode(), buildWavOptimized() (+16 more)

### Community 98 - "home.test.js"
Cohesion: 0.05
Nodes (31): app, assert, authMock, clearedIntervals, clearedTimers, createCollectionCardResult, cuddleResult, doodleApiMock (+23 more)

### Community 99 - ".decide"
Cohesion: 0.25
Nodes (3): After, ReshapeVoucherRule, ReshapeVoucherRuleTest

### Community 100 - "KnowledgeBaseManagement.vue"
Cohesion: 0.05
Nodes (24): fetchDocuments(), fetchSliceCountForSingleDocument(), fetchSliceCountsForDocuments(), handleDelete(), handlePageChange(), handleParse(), handler(), handleSearch() (+16 more)

### Community 101 - "SafetyResult"
Cohesion: 0.12
Nodes (22): AliyunContentSafetyProvider, Any, ContentSafetyProviderBase, ABC, Enum, Evaluate one user input or generated output chunk., SafetyDecision, SafetyDirection (+14 more)

### Community 102 - "PetStoryQueryServiceImplTest"
Cohesion: 0.12
Nodes (8): Override, PageData, PetStoryQueryServiceImpl, SuppressWarnings, PetStoryQueryServiceImplTest, org.junit.jupiter.params.ParameterizedTest, org.junit.jupiter.params.provider.ValueSource, org.mockito.ArgumentCaptor

### Community 103 - "compilerOptions"
Cohesion: 0.05
Nodes (38): compilerOptions, allowJs, allowSyntheticDefaultImports, baseUrl, composite, lib, module, moduleResolution (+30 more)

### Community 104 - "le"
Cohesion: 0.08
Nodes (20): Ar(), Jv(), kv(), le(), Ll(), lQ(), mv(), Pl() (+12 more)

### Community 105 - "AgentSnapshotField"
Cohesion: 0.06
Nodes (30): AgentUpdateDTO, AgentSnapshotField, AGENT_CODE, AGENT_NAME, ASR_MODEL_ID, CHAT_HISTORY_CONF, CONTEXT_PROVIDERS, CORRECT_WORD_FILE_IDS (+22 more)

### Community 106 - "org.springframework.context.annotation.Configuration"
Cohesion: 0.05
Nodes (26): com.aliyun.oss.OSS, com.volcengine.ark.runtime.service.ArkService, jakarta.annotation.PreDestroy, AliyunOssConfig, AliyunOssProperties, RestTemplate, RestTemplateConfig, RedisTemplate (+18 more)

### Community 107 - "RAGFlowAdapter"
Cohesion: 0.11
Nodes (11): KnowledgeFilesDTO, BatchIdReq, InfoVO, ListReq, ObjectMapper, Override, PageData, UpdateReq (+3 more)

### Community 108 - "MCPEndpointClient"
Cohesion: 0.07
Nodes (23): ASRProvider, MCPEndpointClient, any, Exception, Future, MCP接入点客户端，用于管理MCP接入点状态和工具, call_mcp_endpoint_tool(), call_mcp_endpoint_tool (+15 more)

### Community 109 - "pages"
Cohesion: 0.05
Nodes (39): app.json pages, app.json tabBar, pages/settings/settings, pages/welcome/welcome, pages, sitemapLocation, style, tabBar (+31 more)

### Community 110 - "ReplacementWordManagement.vue"
Cohesion: 0.08
Nodes (13): data(), getValidLines(), handleFileChange(), submit(), wordCount(), created(), fetchFileList(), goToPage() (+5 more)

### Community 111 - ".isEnabled"
Cohesion: 0.06
Nodes (5): TreeUtils, Override, OssServiceTest, AgentChatAudioServiceImplTest, MockMultipartFile

### Community 112 - "BigSceneList.vue"
Cohesion: 0.11
Nodes (6): created(), fetchList(), handleDelete(), handleSubmit(), openCreate(), resetForm()

### Community 113 - "GIFEncoder"
Cohesion: 0.09
Nodes (4): ByteArray(), GIFEncoder(), LZWEncoder(), s()

### Community 114 - "index.test.js"
Cohesion: 0.06
Nodes (3): assert, Module, wsSendCalls

### Community 115 - "AgentEntity"
Cohesion: 0.09
Nodes (7): AgentDao, AgentEntity, AgentPluginMapping, AgentServiceImpl, Override, PageData, AgentInfoVO

### Community 116 - "StoryActionImageServiceImpl.java"
Cohesion: 0.05
Nodes (31): DataFormatter, StoryWeightPeriod, AFTERNOON, EVENING, MORNING, NIGHT, ActionDao, ActionImageDao (+23 more)

### Community 117 - "ya"
Cohesion: 0.09
Nodes (3): ow, ya(), ze()

### Community 118 - ".getUserId"
Cohesion: 0.06
Nodes (20): ChatHistoryDeleteService, ChatHistoryExportService, GetMapping, PostMapping, RequestMapping, RestController, PdcNfcWriteJobAdminController, PdcNfcWriteResultImporter (+12 more)

### Community 119 - "backpack.js"
Cohesion: 0.12
Nodes (29): _changeQty(), _decorate(), _emoji(), { get, post }, { getTheme, applyTheme }, loadAll(), logic, onBuyAgain() (+21 more)

### Community 120 - "WebSocketManager"
Cohesion: 0.13
Nodes (10): connect, disconnect, _handleMessage, WebSocket Protocol Dispatcher, _scheduleReconnect, sendAudioFrame, sendHello, sendListenStart (+2 more)

### Community 121 - "MqttGatewayAuthorizationTest"
Cohesion: 0.12
Nodes (10): com.sun.net.httpserver.HttpExchange, com.sun.net.httpserver.HttpHandler, com.sun.net.httpserver.HttpServer, GatewayRequestException, GatewayResponse, MqttGatewayAuthorization, MqttGatewayAuthorizationTest, org.junit.jupiter.params.provider.Arguments (+2 more)

### Community 122 - "NfcWriteJobManagement.vue"
Cohesion: 0.09
Nodes (22): beforeDestroy(), beforeRouteLeave(), canDownload(), canImport(), canManualWrite(), confirmCreate(), created(), fetchBatches() (+14 more)

### Community 123 - "my.js"
Cohesion: 0.09
Nodes (27): applyUserData(), inviteApi, isUsableShareInvite(), loadPetStatus(), loadShareInviteCode(), loadUserProfile(), onNavAccount(), onNavAlbum() (+19 more)

### Community 124 - "AgentSnapshotServiceImplTest"
Cohesion: 0.08
Nodes (5): AgentSnapshotDataDTO, FunctionInfo, AgentUpdateDTOTest, AgentSnapshotServiceImplTest, SuppressWarnings

### Community 126 - "NfcSchemeManagement.vue"
Cohesion: 0.12
Nodes (17): beforeDestroy(), beforeRouteLeave(), created(), fetchBatches(), fetchProgressForBatch(), handleCancel(), handleGenerate(), handlePageChange() (+9 more)

### Community 127 - "CompanionServiceImpl"
Cohesion: 0.04
Nodes (33): CompanionEntity, CompanionServiceImpl, Override, SetupPhase1Result, BirthResult, CompanionBirthCalculator, CompanionLabels, CompanionMood (+25 more)

### Community 128 - "DeviceAddressBookServiceImpl"
Cohesion: 0.13
Nodes (7): DeviceAddressBookDao, DeviceAddressBookEntity, DeviceAddressBookService, DeviceAddressBookServiceImpl, Override, SuppressWarnings, DeviceAddressBookServiceImplTest

### Community 129 - "ActionList.vue"
Cohesion: 0.09
Nodes (22): beforeDestroy(), captionList(), closeCaptionEditor(), closeUpload(), created(), fetchList(), handleDelete(), handleDeleteImage() (+14 more)

### Community 130 - "BaseServiceImplTest"
Cohesion: 0.12
Nodes (14): BatchResult, com.baomidou.mybatisplus.extension.toolkit.SqlHelper, BaseServiceImplTest, CallbackCapturingTestService, Override, SuppressWarnings, TestEntity, TestMapper (+6 more)

### Community 131 - "DeviceManagement.vue"
Cohesion: 0.06
Nodes (30): compareTimestamps(), formatCreateDate(), formatTimestamp(), hasTimestampValue(), parseLegacyDate(), parseTimestamp(), created(), fetchAgentList() (+22 more)

### Community 132 - "pages"
Cohesion: 0.06
Nodes (30): getLocation, pages/settings/settings, pages/welcome/welcome, pages, requiredPrivateInfos, tabBar, backgroundColor, borderStyle (+22 more)

### Community 133 - "login/index.vue"
Cohesion: 0.03
Nodes (52): i18n, login(), configStore, langStore, updateTabBarText(), ImportMeta, ImportMetaEnv, *.vue (+44 more)

### Community 134 - "lombok.extern.slf4j.Slf4j"
Cohesion: 0.03
Nodes (83): com.aliyun.oss.model.CannedAccessControlList, com.baomidou.mybatisplus.core.conditions.query.QueryWrapper, com.baomidou.mybatisplus.core.conditions.update.UpdateWrapper, com.baomidou.mybatisplus.core.metadata.IPage, com.baomidou.mybatisplus.extension.plugins.pagination.Page, com.baomidou.mybatisplus.extension.repository.IRepository, com.baomidou.mybatisplus.spring.repository.CrudRepository, com.fasterxml.jackson.core.type.TypeReference (+75 more)

### Community 135 - "StoryStateSelectorTest"
Cohesion: 0.08
Nodes (22): FunctionalInterface, java.util.Queue, SelectedStoryState, StoryActionCandidate, StoryImageCandidate, StorySceneCandidate, StorySelectionResult, StorySelectionResultType (+14 more)

### Community 136 - "ft_render-ByO_jG18.js"
Cohesion: 0.10
Nodes (8): createNamedFunction(), _environ_get(), _environ_sizes_get(), extendError(), getEnvStrings(), getExecutableName(), makeLegalFunctionName(), writeAsciiToMemory()

### Community 137 - "AudioPlayer.vue"
Cohesion: 0.09
Nodes (17): audioRef, currentTime, duration, formattedCurrentTime, formattedDuration, formatTime(), handleVolumeMouseEnter(), isMuted (+9 more)

### Community 138 - "agent/index.vue"
Cohesion: 0.10
Nodes (15): activeValue, emit, Emits, handleTabClick(), Props, TabItem, chatRef, currentAgentId (+7 more)

### Community 139 - "com.baomidou.mybatisplus.annotation.TableName"
Cohesion: 0.07
Nodes (16): com.baomidou.mybatisplus.annotation.TableName, lombok.EqualsAndHashCode, BaseEntity, AgentTemplateEntity, AgentVoicePrintEntity, AgentTemplateVO, ItemConsumeLogEntity, ItemGrantLogEntity (+8 more)

### Community 140 - "feedback/feedback.js"
Cohesion: 0.12
Nodes (15): FALLBACK_FEEDBACK_TYPES, buildTypeOptions(), { FALLBACK_FEEDBACK_TYPES }, feedbackApi, loadFeedbackTypes(), onAgreeConsent(), onDescriptionInput(), onLoad() (+7 more)

### Community 141 - "voice-call.test.js"
Cohesion: 0.08
Nodes (5): assert, innerAudioContextRecords, Module, originalWx, setInnerAudioOptionCalls

### Community 142 - "GlobalCacheManager"
Cohesion: 0.14
Nodes (10): CacheConfig, CacheType, Enum, GlobalCacheManager, Any, CacheEntry, CacheStrategy, Enum (+2 more)

### Community 143 - "XssHttpServletRequestWrapper"
Cohesion: 0.29
Nodes (5): jakarta.servlet.http.HttpServletRequestWrapper, jakarta.servlet.ServletInputStream, Override, XssHttpServletRequestWrapper, ServletInputStream

### Community 144 - "Builder"
Cohesion: 0.25
Nodes (20): BatchIdReq, BatchOperationVO, CreateReq, DatasetDTO, Edge, GraphVO, InfoVO, AllArgsConstructor (+12 more)

### Community 145 - "DictManagement.vue"
Cohesion: 0.10
Nodes (13): batchDeleteDictData(), batchDeleteDictType(), created(), deleteDictData(), goToPage(), handleDictTypeRowClick(), handlePageSizeChange(), handleSearch() (+5 more)

### Community 146 - "PdcNfcAdminRequestEntity"
Cohesion: 0.16
Nodes (5): PdcNfcAdminRequestEntity, Override, RequestFingerprintTest, PdcNfcAdminIdempotencyServiceTest, TestResponse

### Community 147 - "HeaderBar.vue"
Cohesion: 0.10
Nodes (11): cancel(), resetForm(), changeLanguage(), completeResetCascader(), handleAvatarClick(), handleCascaderChange(), handleLogout(), handleUserMenuVisibleChange() (+3 more)

### Community 148 - "WakewordDetector"
Cohesion: 0.15
Nodes (6): DetectorAssets, DetectorAssetsBuilder, Path, ndarray, WakewordDetector, Any

### Community 149 - "ZA"
Cohesion: 0.13
Nodes (4): ln(), nv(), of, ZA()

### Community 150 - "ModelEditDialog.vue"
Cohesion: 0.13
Nodes (18): ensureObject(), "form.configJson.type"(), formatJson(), getSensitiveFieldName(), handleClose(), handleInputBlur(), handleJsonChange(), handleOpen() (+10 more)

### Community 151 - "VoiceCloneDialog.vue"
Cohesion: 0.10
Nodes (12): audioBufferToWav(), bufferToFile(), drawWaveform(), generateWaveform(), handleDialogClose(), handleFileChange(), handleNext(), handleReset() (+4 more)

### Community 152 - "VoiceCallManager"
Cohesion: 0.12
Nodes (8): connect, getInstance, hangup, _scheduleRecordRestart, setMedia, toggleMute, VoiceCallManager, Voice Call State Machine

### Community 153 - "XssFilter.java"
Cohesion: 0.14
Nodes (15): DelegatingFilterProxy, jakarta.servlet.Filter, jakarta.servlet.FilterChain, jakarta.servlet.FilterConfig, FilterRegistrationBean, XssConfig, Override, XssFilter (+7 more)

### Community 154 - "doodle-canvas.js"
Cohesion: 0.13
Nodes (24): BRUSH_COLORS, BRUSH_SIZES, brushWidthForPixels(), canvas2d, clamp(), createEngine(), createSticker(), createStroke() (+16 more)

### Community 155 - "incubation-scene.js"
Cohesion: 0.10
Nodes (27): applySceneChange(), attached(), canvas2d, clearSceneLoadWatchdog(), deriveClockPosition(), detached(), { EGG_DEPTH_OVERLAY, EGG_SPECULAR_OVERLAY }, loadRemoteImage() (+19 more)

### Community 156 - "incubation-environment.js"
Cohesion: 0.10
Nodes (21): { OSS_SCENE_BASE }, SCENE_OPTIONS, SCENE_TABLE, assert, assets, keys, springClearDay, WINDOW_WEATHER (+13 more)

### Community 157 - "environment-state.js"
Cohesion: 0.14
Nodes (24): companionDay(), hash32(), lightPhaseFromPeriod(), localDateKey(), localDaySerial(), localParts(), millisecondsUntilNextEnvironmentBoundary(), nextEnvironmentBoundary() (+16 more)

### Community 158 - "ModelConfig.vue"
Cohesion: 0.08
Nodes (24): confirm(), 'formData.supplier'(), handleClose(), initConfigJson(), initDynamicConfig(), resetForm(), validateModelId(), visible() (+16 more)

### Community 159 - "destiny.js"
Cohesion: 0.10
Nodes (15): CHARACTERS, closeVoicePicker(), OCCUPATIONS, onNext(), onPlayVoice(), onUnload(), onVoiceOverlayTap(), onVoiceTap() (+7 more)

### Community 160 - "FeedbackServiceImpl"
Cohesion: 0.15
Nodes (7): FeedbackHandleDTO, FeedbackEntity, FeedbackServiceImpl, Override, QueryWrapper, FeedbackAdminVO, FeedbackServiceImplTest

### Community 161 - "小智后台管理系统（Manager API）"
Cohesion: 0.10
Nodes (26): 小智设备管理 Web 前端（Vue2 + Element UI）, 小智语音助手微信小程序前端, 知识卡导出索引文件, 小智语音交互服务器（Xiaozhi Server）, 配置加载与资源管理, Graphify 代码图谱输出产物, 小智 ESP32 语音交互系统（xiaozhi-esp32-server）, 功能实现计划文档集 (+18 more)

### Community 162 - "FunctionDialog.vue"
Cohesion: 0.12
Nodes (8): flushArray(), flushJson(), handleParamChange(), loadFeatureStatus(), loadMcpAddress(), loadMcpTools(), refreshStatus(), value()

### Community 163 - "daily-window-detail.js"
Cohesion: 0.15
Nodes (24): cleanup(), closeImmediately(), configureTopbar(), detached(), drawFrame(), environment(), FEEDBACK_MESSAGES, 'image,weather,season,period,lightPhase'() (+16 more)

### Community 164 - "manager-mobile/package.json"
Cohesion: 0.08
Nodes (24): author, email, gitee, github, name, zhName, bugs, url (+16 more)

### Community 165 - "TtsModel.vue"
Cohesion: 0.12
Nodes (12): deleteRow(), filteredTtsModels(), handler(), handleResponse(), handleScroll(), loadData(), mounted(), saveEdit() (+4 more)

### Community 166 - "pdcNfcState.mjs"
Cohesion: 0.16
Nodes (15): assetId(), fetchDetail(), fetchLogs(), handleLogsPageChange(), safeView(), visible(), batchActions(), capabilityModeLabel() (+7 more)

### Community 167 - "PageData"
Cohesion: 0.08
Nodes (9): PageData, AgentSnapshotController, AgentSnapshotVO, DevicePageUserDTO, PageData, UserShowDeviceListVO, PdcNfcAssetQueryDTO, SysOperationLogDTO (+1 more)

### Community 168 - "test_aliyun_provider.py"
Cohesion: 0.13
Nodes (22): api_reply(), FakeClient, make_aliyun_provider(), parametrize, Catches an API failure accidentally being allowed in enforce mode., Catches non-success or malformed Alibaba responses being allowed., Catches a deployment without optional service keys failing or using wrong APIs., Catches oversized text being sent to Alibaba Cloud. (+14 more)

### Community 169 - "org.springframework.context.annotation.Bean"
Cohesion: 0.21
Nodes (6): io.swagger.v3.oas.models.OpenAPI, SwaggerConfig, KnowledgeBaseConfig, OpenAPI, org.springdoc.core.models.GroupedOpenApi, org.springframework.context.annotation.Bean

### Community 170 - "doodle-canvas.test.js"
Cohesion: 0.08
Nodes (22): afterNew, art, assert, base, clampedSticker, cleaned, cleanExport, ended (+14 more)

### Community 171 - "ConfigService"
Cohesion: 0.10
Nodes (5): LogOperation, ConfigService, ChatQuotaResultVO, SubscriptionController, SysParamsController

### Community 172 - "orders.js"
Cohesion: 0.22
Nodes (13): GET /payment/orders API, _applyFilter(), _decorate(), _formatTime(), { get }, { getTheme, applyTheme }, loadOrders(), onLoad() (+5 more)

### Community 173 - "audio.test.js"
Cohesion: 0.07
Nodes (14): AudioManager, OpusDecoder, OpusEncoder, assert, AudioManager, createdContexts, destroy(), makeRecorderStub() (+6 more)

### Community 174 - "核心能力提供商适配层（ASR/TTS/LLM/Memory/Intent/VAD/VLLM）"
Cohesion: 0.11
Nodes (24): Agent 编排共享工具与插件初始化, Agent 编排核心：消息处理与插件初始化, 文本消息处理器与路由分发, HTTP API 服务与设备接口处理器, 项目文档与部署指南, Graphify 代码图谱输出产物, 小智语音交互服务器（Xiaozhi Server）, WebSocket 连接服务与认证管理 (+16 more)

### Community 175 - "devDependencies"
Cohesion: 0.09
Nodes (23): @babel/plugin-syntax-dynamic-import, @babel/plugin-transform-runtime, compression-webpack-plugin, devDependencies, @babel/plugin-syntax-dynamic-import, @babel/plugin-transform-runtime, compression-webpack-plugin, sass (+15 more)

### Community 176 - ".beforeQuery"
Cohesion: 0.12
Nodes (13): com.baomidou.mybatisplus.extension.plugins.inner.InnerInterceptor, com.baomidou.mybatisplus.extension.plugins.MybatisPlusInterceptor, MybatisPlusInterceptor, MybatisPlusConfig, DataFilterInterceptor, Override, SuppressWarnings, DataScope (+5 more)

### Community 177 - "RuntimeConfig"
Cohesion: 0.18
Nodes (12): main(), AudioSettings, DEFAULT_WAKE_WORDS, DetectorSettings, load_config(), _load_wake_words_from_keywords_file(), LoggingSettings, Path (+4 more)

### Community 178 - "dependencies"
Cohesion: 0.10
Nodes (21): core-js, dotenv, element-ui, flyio, dependencies, core-js, dotenv, element-ui (+13 more)

### Community 179 - "Plugin"
Cohesion: 0.16
Nodes (4): Plugin, Any, PluginManager, Any

### Community 180 - "VoicePrint.vue"
Cohesion: 0.11
Nodes (10): playAudio(), created(), deleteVoicePrint(), fetchVoicePrints(), goToPage(), handlePageSizeChange(), handleSubmit(), initTableColumns() (+2 more)

### Community 181 - "PdcNfcClaimController"
Cohesion: 0.22
Nodes (5): PdcNfcClaimController, PdcNfcClaimConfirmDTO, PdcNfcClaimService, PdcNfcClaimPreviewVO, PdcNfcClaimControllerTest

### Community 182 - "ServerSecretFilter"
Cohesion: 0.28
Nodes (6): jakarta.servlet.ServletRequest, jakarta.servlet.ServletResponse, Override, Oauth2Filter, Override, ServerSecretFilter

### Community 183 - "AgentVoicePrintServiceImpl"
Cohesion: 0.28
Nodes (5): ByteArrayResource, AgentVoicePrintServiceImpl, Override, URI, org.springframework.core.io.ByteArrayResource

### Community 184 - "VoiceCloneServiceImpl"
Cohesion: 0.08
Nodes (14): VoiceDTO, TimbreDao, TimbrePageDTO, TimbreEntity, Override, TimbreServiceImpl, TimbreDetailsVO, VoiceCloneDao (+6 more)

### Community 186 - "UserManagement.vue"
Cohesion: 0.17
Nodes (14): batchDelete(), batchDisable(), batchEnable(), created(), deleteUser(), fetchUsers(), goToPage(), handleBatchDelete() (+6 more)

### Community 187 - "room-clock.js"
Cohesion: 0.23
Nodes (13): attached(), detached(), deviceClock, hide(), onClockTap(), show(), startClock(), stopClock() (+5 more)

### Community 188 - "TTSProvider"
Cohesion: 0.21
Nodes (4): Any, 重写父类方法：使用独立的临时编码器处理音频文件，避免与TTS流式编码器并发冲突。 双流式TTS中，monitor任务在event…, 非流式TTS处理，用于测试及保存音频文件的场景, TTSProvider

### Community 189 - "fg-tabbar.vue"
Cohesion: 0.13
Nodes (12): tabbarList, tabbarStore, cacheTabbarEnable, FgTabBarItem, selectedTabbarStrategy, _tabbar, TABBAR_MAP, tabbarList (+4 more)

### Community 190 - "AgentSnapshotEntity"
Cohesion: 0.10
Nodes (5): AgentSnapshotDao, AgentSnapshotPageDTO, AgentSnapshotEntity, Override, PageData

### Community 192 - "lI"
Cohesion: 0.04
Nodes (35): _A, Ao, CI, dn(), EI(), fn(), gI(), Ir() (+27 more)

### Community 193 - "ItemServiceImpl"
Cohesion: 0.12
Nodes (7): ItemSkuEntity, UserItemEntity, ItemCategory, ItemServiceImpl, Override, ItemSkuVO, UserItemVO

### Community 194 - "CustomDialog.vue"
Cohesion: 0.04
Nodes (17): loadRAGModels(), visible(), addDevice(), closeDialog(), created(), getFirmwareTypes(), submitForm(), created() (+9 more)

### Community 195 - "manifest.json"
Cohesion: 0.10
Nodes (19): appid, description, h5, router, mp-alipay, styleIsolation, usingComponents, mp-baidu (+11 more)

### Community 196 - "EventEmitter"
Cohesion: 0.25
Nodes (6): EventEmitter(), isFunction(), isNumber(), isObject(), isUndefined(), s()

### Community 197 - "buildDiffs"
Cohesion: 0.16
Nodes (20): buildDiffs(), buildSingleConfigItems(), buildSingleFunctionStates(), canonicalField(), correctWordDisplayNames(), displayType(), fieldLabel(), formatDisplayValue() (+12 more)

### Community 198 - "FeedbackService"
Cohesion: 0.14
Nodes (6): FeedbackController, FeedbackSubmitDTO, FeedbackService, FeedbackSubmitVO, FeedbackAdminControllerTest, FeedbackControllerTest

### Community 199 - "PdcNfcWriteCsvExporterTest"
Cohesion: 0.17
Nodes (3): PdcNfcWriteCsvExporter, PdcNfcWriteCsvRow, PdcNfcWriteCsvExporterTest

### Community 200 - "NfcBatchManagement.vue"
Cohesion: 0.18
Nodes (15): beforeDestroy(), confirmCreateWriteJob(), created(), fetchBatches(), fetchSchemeProgress(), handleCancelScheme(), handleGenerateScheme(), handlePageChange() (+7 more)

### Community 201 - "PetStoryStateEntity"
Cohesion: 0.10
Nodes (17): StoryImageTimeOfDay, DAY, NIGHT, SUNSET, StoryRuntimeStatus, ACTIVE, UNINITIALIZED, PetStoryHistoryDao (+9 more)

### Community 202 - "pages/profile/profile.js"
Cohesion: 0.13
Nodes (25): _ensureAgeRange(), { API_BASE_URL }, auth, formatDisplay(), GENDER_LIST, GENDER_MAP, GENDER_REVERSE, loadProfile() (+17 more)

### Community 203 - "applyTheme"
Cohesion: 0.06
Nodes (39): GET /subscription/entitlements API, ReshapeConfirm Component, Backpack Page, ChangeOccupation Page, ChangeRole Page, ChangeSoul Page, onShow(), codes (+31 more)

### Community 204 - "pet-store-actions.test.js"
Cohesion: 0.17
Nodes (7): actionCalls, actionResponses, assert, Module, petStore, storage, updateNicknameCalls

### Community 205 - "permissions"
Cohesion: 0.11
Nodes (18): permissions, <uses-feature android:name=\"android.hardware.camera\"/>, <uses-feature android:name=\"android.hardware.camera.autofocus\"/>, <uses-permission android:name=\"android.permission.ACCESS_COARSE_LOCATION\"/>, <uses-permission android:name=\"android.permission.ACCESS_FINE_LOCATION\"/>, <uses-permission android:name=\"android.permission.ACCESS_NETWORK_STATE\"/>, <uses-permission android:name=\"android.permission.ACCESS_WIFI_STATE\"/>, <uses-permission android:name=\"android.permission.CAMERA\"/> (+10 more)

### Community 206 - "ChatHistoryDialog.vue"
Cohesion: 0.06
Nodes (18): assert, componentPath, context(), deferred(), fs, Module, path, run() (+10 more)

### Community 207 - "subscription.js"
Cohesion: 0.12
Nodes (30): _onPlayTap(), _playAudio(), _playLocalFile(), { post, getBaseUrl }, _stopAudio(), buildFeatureTable(), _createSubscriptionOrder(), _doSignContract() (+22 more)

### Community 208 - "AudioManager"
Cohesion: 0.11
Nodes (10): { getOpusRuntime }, OpusDecoder, buildStubRuntime(), buildWasmRuntime(), createWasmImports(), getOpusRuntime(), loadWasmRuntime(), normalizeExports() (+2 more)

### Community 211 - "KnowledgeBaseAdapter"
Cohesion: 0.08
Nodes (5): BatchIdReq, InfoVO, UpdateReq, KnowledgeBaseAdapter, KnowledgeBaseAdapterFactory

### Community 212 - "DateUtils"
Cohesion: 0.27
Nodes (5): Closeable, DateUtils, AddGroup, DefaultGroup, UpdateGroup

### Community 213 - "WechatPayClient"
Cohesion: 0.06
Nodes (22): com.wechat.pay.java.core.notification.NotificationParser, com.wechat.pay.java.service.payments.jsapi.JsapiServiceExtension, com.wechat.pay.java.service.refund.RefundService, Override, MockWechatPayClient, NotifyResult, PrepayRequest, PrepayResult (+14 more)

### Community 214 - "FeatureManagement.vue"
Cohesion: 0.19
Nodes (8): beforeDestroy(), created(), getFeaturesByIds(), handleReset(), handleSave(), loadFeatures(), removeConfigChangeListener(), setupConfigChangeListener()

### Community 215 - "registerType"
Cohesion: 0.21
Nodes (17): __embind_register_bool(), __embind_register_emval(), __embind_register_float(), __embind_register_integer(), __embind_register_memory_view(), __embind_register_std_string(), __embind_register_std_wstring(), __embind_register_void() (+9 more)

### Community 216 - "TTSProvider"
Cohesion: 0.21
Nodes (4): Any, 非流式TTS处理，用于测试及保存音频文件的场景, 重写父类方法：使用独立的临时编码器处理音频文件，避免与TTS流式编码器并发冲突。 双流式TTS中，monitor任务在event…, TTSProvider

### Community 217 - "lombok.NoArgsConstructor"
Cohesion: 0.11
Nodes (11): lombok.Builder, lombok.NoArgsConstructor, AgentChatTitleEntity, PdcNfcReleaseEvidenceDTO, MemoryEntity, UserProfileEntity, PageData, ChatHistoryVO (+3 more)

### Community 218 - "ServerMCPClient"
Cohesion: 0.08
Nodes (19): ElicitationFnT, Implementation, ListRootsFnT, LoggingFnT, LoggingMessageNotificationParams, Any, 调用指定工具 Args: name: 工具名称 arguments: 工具参数 read_timeout_seconds:…, 检查MCP客户端是否连接正常 Returns: bool: 如果客户端已连接并正常工作，返回True，否则返回False (+11 more)

### Community 219 - "SimpleHttpServer.start"
Cohesion: 0.13
Nodes (16): BaseHandler._add_cors_headers, BaseHandler.handle_options, SimpleHttpServer._get_websocket_url, SimpleHttpServer.start, _is_higher_version, OTAHandler._get_websocket_url, OTAHandler._refresh_bin_cache_if_needed, OTAHandler.generate_password_signature (+8 more)

### Community 220 - "agentSnapshotDisplayUtils.mjs"
Cohesion: 0.32
Nodes (11): isPlainObject(), isSensitiveSnapshotKey(), isSnapshotUrlValue(), isWebhookSemanticKey(), normalizeSnapshotOrderedValue(), redactGenericCapabilityPath(), redactSnapshotCapabilityPath(), redactSnapshotDisplayValue() (+3 more)

### Community 221 - "setup_logging"
Cohesion: 0.02
Nodes (74): ABC, formatter(), 为没有 tag 的日志添加默认值，并处理动态模块字符串, 从配置文件中读取日志配置，并设置日志输出格式和级别, setup_logging(), AccessToken, ASRProvider, ASRProviderBase (+66 more)

### Community 222 - "verify-project.js"
Cohesion: 0.20
Nodes (8): app, errors, fs, homeSource, miniprogram, path, project, root

### Community 223 - "IntimacyLevel.java"
Cohesion: 0.19
Nodes (10): clamp(), IntimacyLevel, ACQUAINTED, AMBIGUOUS, CRUSH, DEEP_LOVE, LOVER, next() (+2 more)

### Community 224 - "NfcAssetManagement.vue"
Cohesion: 0.17
Nodes (13): buildParams(), created(), fetchAssets(), fetchPrototypes(), generateUUID(), handleBulkActivate(), handleBulkDisable(), handleBulkScrap() (+5 more)

### Community 226 - "org.apache.shiro.authc.AuthenticationToken"
Cohesion: 0.21
Nodes (5): Override, Oauth2Token, Override, ServerSecretToken, org.apache.shiro.authc.AuthenticationToken

### Community 227 - "Selected Module Pipeline"
Cohesion: 0.13
Nodes (16): ChatGLM LLM, Edge TTS, FunASR Local ASR, No Memory Module, Configured Plugins, PowerMem Memory Module, Selected Module Pipeline, SenseVoiceEncoderSmall (+8 more)

### Community 228 - "WakeupWordsConfig"
Cohesion: 0.22
Nodes (3): FileLock, 生成音频文件路径，使用voice的哈希值作为文件名, WakeupWordsConfig

### Community 229 - "HatchActionServiceImplTest"
Cohesion: 0.22
Nodes (3): HatchActionServiceImpl, Override, HatchActionServiceImplTest

### Community 231 - "StreamTTSPerformanceTester"
Cohesion: 0.14
Nodes (11): main(), 测试火山引擎流式TTS首词延迟（测试多次取平均）, 测试阿里云流式TTS首词延迟（测试多次取平均）, 测试PaddleSpeech流式TTS首词延迟（测试多次取平均）, 测试IndexStream流式TTS首词延迟（测试多次取平均）, 测试Linkerai流式TTS首词延迟（测试多次取平均）, 测试讯飞流式TTS首词延迟（测试多次取平均）, 计算测试结果（正确处理None值，剔除失败测试） (+3 more)

### Community 232 - "app.test.js"
Cohesion: 0.12
Nodes (11): appPath, assert, auth, captureCalls, Module, nfcClaimIntent, parsedEntryOptions, request (+3 more)

### Community 233 - "VoiceCloneManagement.vue"
Cohesion: 0.08
Nodes (25): formatDate(), formatFileSize(), created(), deleteParam(), deleteSelectedParams(), fetchFirmwareList(), getFirmwareTypes(), goToPage() (+17 more)

### Community 234 - "account.test.js"
Cohesion: 0.14
Nodes (9): accountPath, app, assert, callOrder, makePage(), modalResult, Module, petStore (+1 more)

### Community 235 - "time-service.js"
Cohesion: 0.48
Nodes (5): beijingDateKey(), formatBeijingDate(), now(), requireAuthoritative(), sync()

### Community 236 - "LLMProvider"
Cohesion: 0.23
Nodes (6): GenerateContentResponse, LLMProvider, Any, 分别测试 HTTP 和 HTTPS 代理是否可用，并设置环境变量。 如果 HTTPS 代理不可用但 HTTP 可用，会将 HTTPS_PROXY 也指向…, setup_proxy_env(), test_proxy()

### Community 237 - "Vue CLI Build Configuration"
Cohesion: 0.14
Nodes (15): Element UI Component Library, flyio HTTP Client, Vue.js 2 Framework, vue-i18n Internationalization, xiaozhi Web Manager Project, CacheViewer Component, CDN Enabled Runtime Flag, App Root Component (+7 more)

### Community 239 - "PdcNfcClaimPreviewServiceTest"
Cohesion: 0.11
Nodes (4): Override, PdcNfcClaimResultVO, PdcNfcClaimPreviewServiceTest, PdcNfcClaimServiceTest

### Community 240 - "NfcActivationScanner.vue"
Cohesion: 0.17
Nodes (11): data(), focusInput(), handleClear(), handleRemove(), handleScan(), mounted(), reset(), acceptScan() (+3 more)

### Community 241 - "add-device.test.js"
Cohesion: 0.16
Nodes (10): assert, authMock, fs, makePage(), Module, pagePath, path, resetScenario() (+2 more)

### Community 242 - "window-weather-canvas.js"
Cohesion: 0.25
Nodes (11): clipWindowGlass(), drawCloudDrift(), drawDroplets(), drawFog(), drawFrame(), drawIceGlints(), drawLeaves(), drawLightning() (+3 more)

### Community 243 - "SubscriptionServiceImpl"
Cohesion: 0.14
Nodes (8): SubscriptionPlanEntity, UserSubscriptionEntity, Override, SubscriptionServiceImpl, EntitlementVO, BonusItem, SubscriptionPlanVO, UserSubscriptionVO

### Community 244 - "mp-weixin"
Cohesion: 0.14
Nodes (14): getLocation, mp-weixin, appid, optimization, permission, requiredPrivateInfos, setting, usingComponents (+6 more)

### Community 245 - "buildSavedVersionDiffSnapshot"
Cohesion: 0.24
Nodes (11): buildSavedVersionDiffSnapshot(), buildSingleSnapshotDetail(), buildVersionDiffSnapshot(), fetchNearestPreviousSnapshotDetail(), fetchPreviousSnapshotDetail(), fetchPreviousSnapshotDetailWithoutId(), fetchSnapshotDetail(), fetchSnapshotDetailByVersion() (+3 more)

### Community 246 - "remote-image.test.js"
Cohesion: 0.18
Nodes (9): resolveStoryBackground(), cache, inflight, loadRemoteImage(), assert, calls, deferred, { loadRemoteImage } (+1 more)

### Community 248 - "Multi-Level Memory"
Cohesion: 0.16
Nodes (14): Cognitive Memory System Architecture, Episodic Memory, MemoryProviderFactory, Cognitive Memory Framework, Knowledge Graph Store, Hybrid Retrieval Engine, Multi-Level Memory, OceanBase Database (+6 more)

### Community 249 - "OtaEntity"
Cohesion: 0.21
Nodes (7): OtaDao, OtaEntity, Override, PageData, QueryWrapper, OtaServiceImpl, OtaService

### Community 250 - "_common.sh"
Cohesion: 0.23
Nodes (8): build-push-manager-api.sh script, build-push-manager-web.sh script, build-push-xiaozhi-server.sh script, _acr_login(), init(), _prepare_builder(), _common.sh script, mirror-funasr-image.sh script

### Community 251 - "invite-codes.test.js"
Cohesion: 0.29
Nodes (5): apiCalls, assert, inviteApi, inviteCodesPath, Module

### Community 252 - "profile.test.js"
Cohesion: 0.17
Nodes (10): api, assert, auth, modalCalls, Module, petStore, profilePath, putCalls (+2 more)

### Community 253 - ".validateEntity"
Cohesion: 0.10
Nodes (8): SysDictTypeController, SysDictDataDTO, SysDictTypeDTO, SysDictTypeService, SysDictTypeVO, TimbreDataDTO, org.springframework.context.support.ResourceBundleMessageSource, ResourceBundleMessageSource

### Community 254 - "ASRProvider"
Cohesion: 0.21
Nodes (7): ASRProvider, parse_response(), Generate protocol header., Construct the request payload., Send request to Volcano ASR service., slice data :param data: wav data :param chunk_size: the segment size in one…, protocol_version(4 bits), header_size(4 bits), message_type(4 bits),…

### Community 255 - "PetMood"
Cohesion: 0.18
Nodes (10): PetMood, ANXIETY, CALM, CARE, CURIOSITY, EXCITEMENT, FATIGUE, FRUSTRATION (+2 more)

### Community 257 - "ProviderDialog.vue"
Cohesion: 0.17
Nodes (10): addField(), batchRemoveFields(), forceTableRerender(), handleClose(), removeField(), resetForm(), stopEditing(), submit() (+2 more)

### Community 258 - "TTSProvider"
Cohesion: 0.06
Nodes (21): Any, 重写父类方法：使用独立的临时编码器处理音频文件，避免与TTS流式编码器并发冲突。 双流式TTS中，monitor任务在event…, 非流式生成音频数据，用于生成音频及测试场景, 确保WebSocket连接可用，支持60秒内连接复用, TTSProvider, 流式处理TTS音频，每句只推送一次音频列表, 音频文件转换为PCM编码，使用24kHz采样率, 音频文件转换为Opus编码，使用24kHz采样率和自己的编码器 (+13 more)

### Community 259 - "PaymentNotifyController.java"
Cohesion: 0.24
Nodes (5): MockPaymentNotifyController, PaymentNotifyController, PayChannel, PaymentNotifyService, PaymentNotifyService

### Community 260 - "Agent Base Prompt Template"
Cohesion: 0.15
Nodes (13): Backup Chat Style Policy, Backup Agent Base Prompt Template, get_weather Tool, Memory Usage Policy, Boundary Safety and Compliance Policy, Agent Base Prompt Template, Tool and Knowledge Policy, TTS Format Constraints (+5 more)

### Community 261 - "PdcNfcSchemeAdminController"
Cohesion: 0.18
Nodes (7): GetMapping, PostMapping, RequestMapping, RestController, PdcNfcSchemeAdminController, PdcNfcSchemeJobService, PdcNfcSchemeProgressVO

### Community 262 - "chat_integration_review_cases.py"
Cohesion: 0.27
Nodes (8): _base(), RecursiveResponseCreationFailureLLM, ResponseCreationFailureLLM, SingleToolCallLLM, test_blocked_input_clears_stale_abort_before_trusted_speech(), test_chat_debug_logs_only_redacted_operational_metadata(), test_recursive_response_creation_error_discards_parent_gate_buffer(), test_top_level_response_creation_error_uses_trusted_fallback_and_ends_once()

### Community 263 - "java.lang.annotation.Documented"
Cohesion: 0.61
Nodes (5): java.lang.annotation.Documented, java.lang.annotation.Retention, java.lang.annotation.Target, DataFilter, OperationLog

### Community 264 - "HatchActionType"
Cohesion: 0.18
Nodes (8): from(), HatchActionType, CUDDLE, DOODLE, LESSON, NICKNAME, WISH, SuppressWarnings

### Community 265 - "ContextProviderDTO"
Cohesion: 0.14
Nodes (9): java.sql.CallableStatement, java.sql.PreparedStatement, java.sql.ResultSet, ContextProviderDTO, ContextProviderListTypeHandler, Override, ContextProviderListTypeHandlerTest, org.apache.ibatis.type.BaseTypeHandler (+1 more)

### Community 266 - "com.fasterxml.jackson.annotation.JsonIgnoreProperties"
Cohesion: 0.25
Nodes (15): com.fasterxml.jackson.annotation.JsonIgnoreProperties, com.fasterxml.jackson.annotation.JsonInclude, DeleteItem, DocAggVO, HitVO, AllArgsConstructor, Data, NoArgsConstructor (+7 more)

### Community 267 - "alova.ts"
Cohesion: 0.09
Nodes (24): alovaInstance, langMap, { onAuthRequired, onResponseRefreshToken }, ContentTypeEnum, ResultEnum, ShowMessage(), IResponse, PageParams (+16 more)

### Community 268 - "ChatHistoryExportServiceImpl"
Cohesion: 0.13
Nodes (11): com.aliyun.dm20151123.Client, com.aliyun.dm20151123.models.SingleSendMailResponse, com.aliyun.teautil.models.RuntimeOptions, AgentSessions, ChatHistoryExportServiceImpl, Override, EmailSendDTO, EmailService (+3 more)

### Community 269 - "daily-window-detail.test.js"
Cohesion: 0.14
Nodes (11): assert, componentPath, deferredDownloads, downloadCalls, events, failInstance, fallbackInstance, Module (+3 more)

### Community 270 - "app-plus"
Cohesion: 0.17
Nodes (12): app-plus, compatible, compilerVersion, modules, nvueStyleCompiler, splashscreen, usingComponents, ignoreVersion (+4 more)

### Community 271 - "createWasm"
Cohesion: 0.21
Nodes (12): abort(), addOnInit(), addRunDependency(), createWasm(), getBinary(), getBinaryPromise(), getValue(), intArrayFromBase64() (+4 more)

### Community 272 - "AgentChatHistoryServiceImpl"
Cohesion: 0.11
Nodes (4): AgentChatHistoryServiceImpl, Override, PageData, AgentChatHistoryServiceImplTest

### Community 273 - "TtsAdvancedSettings.vue"
Cohesion: 0.21
Nodes (5): fetchReplacementWordList(), handleCancel(), handleClose(), handleSave(), visible()

### Community 274 - "AgentChatSummaryServiceImpl"
Cohesion: 0.15
Nodes (3): AgentChatHistoryDTO, AgentChatSummaryServiceImpl, Override

### Community 275 - "AgentTemplateManagement.vue"
Cohesion: 0.17
Nodes (12): batchDeleteTemplate(), created(), DEFAULT_MODEL_CONFIG, deleteTemplate(), editTemplate(), fetchTemplateById(), handleDialogConfirm(), handlePageChange() (+4 more)

### Community 276 - "Subscription Page (我的契约)"
Cohesion: 0.29
Nodes (7): Bond Panel (羁绊面板), Subscription Page (我的契约), Subscription Payment Flow, Plan Comparison Table, 401 Silent Re-login, HTTP Request Utility, Manager API Base URL

### Community 277 - "nfc-claim.test.js"
Cohesion: 0.14
Nodes (13): app, assert, authMock, makePage(), Module, nfcClaimApiMock, nfcIntentMock, pagePath (+5 more)

### Community 278 - "ShiroConfig.java"
Cohesion: 0.16
Nodes (12): AuthorizationAttributeSourceAdvisor, DefaultWebSessionManager, LifecycleBeanPostProcessor, ShiroConfig, Oauth2Filter, org.apache.shiro.mgt.SecurityManager, org.apache.shiro.session.mgt.SessionManager, org.apache.shiro.spring.LifecycleBeanPostProcessor (+4 more)

### Community 279 - "chat.test.js"
Cohesion: 0.18
Nodes (9): assert, BASE, lastYear, Module, now, nowDate, sameYear, todayLabel (+1 more)

### Community 280 - "welcome.test.js"
Cohesion: 0.19
Nodes (11): app, assert, authMock, makePage(), Module, nfcIntentMock, redirectTo(), resetScenario() (+3 more)

### Community 281 - "CaptchaServiceImpl"
Cohesion: 0.14
Nodes (7): com.aliyun.dysmsapi20170525.Client, com.google.common.cache.Cache, CaptchaServiceImpl, Override, Client, Override, RuntimeOptions

### Community 284 - "manager-web/package.json"
Cohesion: 0.25
Nodes (7): browserslist, name, private, version, > 1%, last 2 versions, not dead

### Community 285 - "LLMPerformanceTester"
Cohesion: 0.29
Nodes (3): create_instance(), LLMPerformanceTester, main()

### Community 286 - "floating-call-ball.js"
Cohesion: 0.25
Nodes (4): attached(), formatDuration(), _sync(), VoiceCallManager

### Community 288 - "微信支付改造三道防线"
Cohesion: 0.24
Nodes (10): 回调失败导致订单错误超时风险, PENDING 订单, 微信支付现有纯回调模式, retryPaidOrders, 订阅履约, 前端主动查单, 支付幂等防重, 定时任务兜底补偿 (+2 more)

### Community 289 - "incubation-scene.test.js"
Cohesion: 0.11
Nodes (16): assert, componentPath, deferredDownloads, downloadCalls, events, fs, hangingInstance, instance (+8 more)

### Community 290 - "collection-card.test.js"
Cohesion: 0.20
Nodes (8): assert, fs, Module, pagePath, path, script, styles, template

### Community 291 - "my.test.js"
Cohesion: 0.20
Nodes (4): assert, inviteMineQueue, Module, pagePath

### Community 292 - "ipad"
Cohesion: 0.20
Nodes (10): ipad, app, app@2x, notification, notification@2x, proapp@2x, settings, settings@2x (+2 more)

### Community 293 - "OpusEncoderUtils"
Cohesion: 0.17
Nodes (8): Encoder, OpusEncoderUtils, Any, ndarray, Opus编码工具类 将PCM音频数据编码为Opus格式, 将字节数组转换为short数组 (16位PCM), 初始化Opus编码器 Args: sample_rate: 采样率 (Hz) channels: 通道数 (1=单声道, 2=立体声)…, 将PCM数据编码为Opus格式，以流式方式进行处理 Args: pcm_data: PCM字节数据 end_of_stream: 是否为流的结束,…

### Community 295 - "voice-call-manager.js"
Cohesion: 0.25
Nodes (6): FloatingCallBall Component, Voice Call Media Pipeline, Voice Call Page, Voice Call Ringback Tone, assert, VoiceCallManager

### Community 296 - "VoiceCloneService"
Cohesion: 0.14
Nodes (4): VoiceCloneController, VoiceCloneDTO, VoiceCloneResponseDTO, VoiceCloneService

### Community 297 - "VoiceResourceManagement.vue"
Cohesion: 0.11
Nodes (11): fetchPlatformList(), handleOpen(), created(), deleteSelectedVoiceClones(), deleteVoiceClone(), fetchVoiceCloneList(), goToPage(), handlePageSizeChange() (+3 more)

### Community 298 - "pet-mood-tab.js"
Cohesion: 0.43
Nodes (5): clearTransitionTimers(), collapse(), detached(), onToggle(), reveal()

### Community 299 - "NfcProductTypeManagement.vue"
Cohesion: 0.20
Nodes (8): registerReleaseEvidence(), buildReleaseEvidencePayload(), buildReleaseEvidenceViewModel(), created(), fetchProductTypes(), releaseEvidenceView(), submitEvidence(), loadPdcNfcApi()

### Community 300 - "doodle-save.test.js"
Cohesion: 0.22
Nodes (6): assert, calls, doodleApi, Module, petStore, saveResult

### Community 302 - "distribute"
Cohesion: 0.22
Nodes (9): abiFilters, minSdkVersion, targetSdkVersion, distribute, android, ios, sdkConfigs, arm64-v8a (+1 more)

### Community 303 - "iphone"
Cohesion: 0.22
Nodes (9): iphone, app@2x, app@3x, notification@2x, notification@3x, settings@2x, settings@3x, spotlight@2x (+1 more)

### Community 304 - "ccall"
Cohesion: 0.25
Nodes (9): ccall(), cwrap(), _emscripten_run_script(), getCFunc(), stringToUTF8(), stringToUTF8Array(), UTF8ArrayToString(), UTF8ToString() (+1 more)

### Community 305 - "check-i18n.js"
Cohesion: 0.31
Nodes (8): diffKeys(), formatKeyList(), fs, locales, main(), path, readLocale(), rootDir

### Community 306 - "functionParamRows"
Cohesion: 0.21
Nodes (13): buildFunctionChanges(), buildFunctionState(), changedFunctionParamKeys(), createFunctionChange(), functionDisplayName(), functionFields(), functionParamLabel(), functionParamRows() (+5 more)

### Community 307 - "date.js"
Cohesion: 0.39
Nodes (7): formatTime(), formatAddDate(), formatDate(), formatDateTool(), isDate(), padLeftZero(), toDate()

### Community 308 - "router/index.js"
Cohesion: 0.14
Nodes (14): canAccessRoute(), readStoredUserInfo(), protectedRoutes, routes, applyTemplateData(), createNewTemplate(), DEFAULT_MODEL_CONFIG, fetchTemplateById() (+6 more)

### Community 309 - "post"
Cohesion: 0.15
Nodes (18): POST /companion/setup API, needReconnectAfterReshape global flag, App globalData, confirmDelete(), onDelete(), onExport(), request, post() (+10 more)

### Community 310 - "DeviceEntity"
Cohesion: 0.17
Nodes (10): DeviceEntity, Override, WebMvcConfig, DeviceTimeSerializationTest, MappingJackson2HttpMessageConverter, org.springframework.http.converter.HttpMessageConverter, org.springframework.http.converter.json.MappingJackson2HttpMessageConverter, org.springframework.web.servlet.config.annotation.CorsRegistry (+2 more)

### Community 311 - "PdcNfcWriteJobDownloadTest"
Cohesion: 0.12
Nodes (3): PdcNfcWriteFile, PdcNfcWriteJobVO, PdcNfcWriteJobDownloadTest

### Community 312 - "android"
Cohesion: 0.25
Nodes (8): hdpi, xhdpi, xxhdpi, xxxhdpi, icons, android, ios, appstore

### Community 313 - "vue.config.js"
Cohesion: 0.22
Nodes (7): cdnResources, CompressionPlugin, { defineConfig }, dotenv, { InjectManifest }, path, TerserPlugin

### Community 314 - "AgentMcpAccessPointServiceImpl"
Cohesion: 0.25
Nodes (5): java.net.URI, XiaoZhiMcpJsonRpcJson, AgentMcpAccessPointServiceImpl, Override, URI

### Community 315 - "KnowledgeFilesController"
Cohesion: 0.18
Nodes (8): BatchIdReq, DeleteMapping, GetMapping, PostMapping, RequestMapping, RestController, KnowledgeFilesController, BatchIdReq

### Community 316 - "websocket Utility"
Cohesion: 0.29
Nodes (7): config/api 配置, xiaozhi-server 聊天服务, opus-decoder 库, audio Utility, ota Utility, request Utility, websocket Utility

### Community 317 - "SAE Startup Wrapper Script"
Cohesion: 0.29
Nodes (7): app.py Entry Point, FunASR Pipeline, SenseVoiceSmall ASR Model Configuration, ConfigMap Fallback Configuration, NAS Mount Configuration Source, SAE Startup Wrapper Script, VAD Model Presence Check

### Community 318 - "intentHandler.py"
Cohesion: 0.05
Nodes (62): report(), handleAbortMessage(), checkWakeupWords(), wakeupWordsResponse(), analyze_intent_with_llm(), check_direct_exit(), handle_user_intent(), _output_block_message() (+54 more)

### Community 319 - "UploadScene.java"
Cohesion: 0.33
Nodes (3): fromCode(), UploadScene, DOODLE

### Community 320 - "fetchSnapshots"
Cohesion: 0.18
Nodes (12): applyPreviousChangedFields(), canDeleteSnapshot(), currentVersionNo(), decorateSnapshotRows(), deleteSnapshot(), fetchSnapshotRowBeforeVersion(), fetchSnapshotRows(), fetchSnapshots() (+4 more)

### Community 321 - "scripts"
Cohesion: 0.25
Nodes (8): scripts, analyze, build, check:i18n, serve, test:snapshot, test:unit, verify:nfc-coverage

### Community 323 - "VADProviderBase"
Cohesion: 0.21
Nodes (5): ABC, VADProviderBase, 释放连接的 VAD 资源（连接关闭时调用）, VADProvider, create_instance()

### Community 326 - "deregister.js"
Cohesion: 0.47
Nodes (3): onCancelDeregister(), onConfirmDeregister(), onShow()

### Community 328 - "AgentVoicePrintVO"
Cohesion: 0.20
Nodes (4): AgentVoicePrintSaveDTO, AgentVoicePrintUpdateDTO, AgentVoicePrintService, AgentVoicePrintVO

### Community 329 - "TestRuntimeHttpServer"
Cohesion: 0.20
Nodes (3): Path, TestRuntimeHttpServer, ThreadingHTTPServer

### Community 331 - "lombok.Getter"
Cohesion: 0.23
Nodes (15): lombok.Getter, lombok.Setter, DeviceRegisterDTO, Application, BoardInfo, ChipInfo, DeviceReportReqDTO, OtaInfo (+7 more)

### Community 332 - "XssUtils"
Cohesion: 0.53
Nodes (3): XssUtils, org.jsoup.safety.Safelist, Safelist

### Community 333 - "SmallSceneList.vue"
Cohesion: 0.14
Nodes (14): bigSceneId(), buildEmptyForm(), created(), data(), fetchList(), fetchWeightSummary(), handleDelete(), handleSubmit() (+6 more)

### Community 334 - "submitRestoreSnapshot"
Cohesion: 0.60
Nodes (5): confirmRestoreSnapshot(), invalidateRestorePreview(), restoreFailedMessage(), submitRestoreSnapshot(), hasValidCurrentStateToken()

### Community 335 - "callRuntimeCallbacks"
Cohesion: 0.22
Nodes (11): addOnPostRun(), addOnPreRun(), callRuntimeCallbacks(), getWasmTableEntry(), initRuntime(), invoke_iii(), invoke_iiii(), invoke_iiiii() (+3 more)

### Community 336 - "normalizeValueForField"
Cohesion: 0.33
Nodes (7): isEquivalentValue(), isPlainObject(), isSameFieldValue(), isSameValue(), normalizeDefaultTtsNumber(), normalizeValue(), normalizeValueForField()

### Community 338 - "SysParamsServiceImpl"
Cohesion: 0.07
Nodes (10): CrudService, CrudServiceImpl, Override, SuppressWarnings, SysParamsDTO, Override, QueryWrapper, SysParamsServiceImpl (+2 more)

### Community 339 - "org.junit.jupiter.api.Test"
Cohesion: 0.03
Nodes (16): AESUtils, CreatePdcNfcBatchDTO, PetAdoptDTO, Override, AESUtilsTest, CompanionMoodTest, IntimacyLevelTest, InviteCodeGeneratorTest (+8 more)

### Community 340 - "ServerActionEnum"
Cohesion: 0.14
Nodes (15): com.fasterxml.jackson.annotation.JsonCreator, com.fasterxml.jackson.annotation.JsonValue, ServerActionPayloadDTO, ServerActionResponseDTO, fromValue(), getValue(), ServerActionEnum, RESTART (+7 more)

### Community 341 - "nfc-claim.js"
Cohesion: 0.07
Nodes (38): onAuthorizePhone(), applyPreview(), auth, doConfirm(), fallbackUuid(), generateRequestId(), { getPendingNfcClaimIntent, clearPendingNfcClaimIntent }, goHomeWithPet() (+30 more)

### Community 343 - "service-worker.js"
Cohesion: 0.50
Nodes (3): CDN_CSS, CDN_JS, isCDNEnabled

### Community 345 - "p3.py"
Cohesion: 0.40
Nodes (4): decode_opus_from_bytes(), decode_opus_from_file(), 从p3二进制数据中解码 Opus 数据，并返回一个 Opus 数据包的列表以及总时长。, 从p3文件中解码 Opus 数据，并返回一个 Opus 数据包的列表以及总时长。

### Community 346 - "performance_tester.py"
Cohesion: 0.70
Nodes (4): get_module_description(), list_performance_tester_modules(), load_and_execute_module(), main()

### Community 347 - "项目文档与部署指南"
Cohesion: 0.50
Nodes (5): 阿里云 ACR 镜像构建与推送执行脚本, scripts/, docs/, 项目文档与部署指南, xiaozhi_server

### Community 348 - "egg-avatar.js"
Cohesion: 0.83
Nodes (3): attached(), compute(), size()

### Community 349 - "provider.vue"
Cohesion: 0.21
Nodes (6): Providers, goBack(), handleConfirm(), localProviders, providerStore, useProvider

### Community 351 - "NfcOperationLogManagement.vue"
Cohesion: 0.24
Nodes (9): operationTypeOptions(), buildParams(), created(), data(), fetchLogs(), handlePageChange(), handleResetFilters(), handleSearch() (+1 more)

### Community 352 - "postupgrade.js"
Cohesion: 0.50
Nodes (3): dependencies, { exec }, TODO: 如果不需要某个平台的小程序，请手动删除或注释掉

### Community 353 - "typings.d.ts"
Cohesion: 0.50
Nodes (3): IResData, IUniUploadFileOptions, IUserInfo

### Community 355 - "emscripten_realloc_buffer"
Cohesion: 0.50
Nodes (4): alignUp(), emscripten_realloc_buffer(), _emscripten_resize_heap(), updateGlobalBufferAndViews()

### Community 356 - "Chinese FST Commands"
Cohesion: 0.50
Nodes (4): Chinese Air Conditioner Controls, Chinese FST Commands, English FST Commands, English Smart Home Controls

### Community 358 - "OpenAI LLMProvider.response"
Cohesion: 0.83
Nodes (4): OpenAI LLMProvider._apply_thinking_disabled, OpenAI LLMProvider.normalize_dialogue, OpenAI LLMProvider.response, OpenAI LLMProvider.response_with_functions

### Community 359 - "UnifiedToolHandler._initialize"
Cohesion: 0.50
Nodes (4): UnifiedToolHandler.current_support_functions, UnifiedToolHandler._initialize, UnifiedToolHandler._initialize_home_assistant, UnifiedToolHandler._initialize_mcp_endpoint

### Community 360 - "MCP Server Settings Template"
Cohesion: 0.50
Nodes (4): MCP Server Settings Template, Filesystem MCP Server, Home Assistant MCP Server, Playwright MCP Server

### Community 361 - "OpusEncoderUtils.encode_pcm_to_opus_stream"
Cohesion: 0.50
Nodes (4): OpusEncoderUtils._convert_bytes_to_shorts, OpusEncoderUtils._encode, OpusEncoderUtils._validate_pcm_data, OpusEncoderUtils.encode_pcm_to_opus_stream

### Community 362 - "Live2D Cubism 示例模型"
Cohesion: 1.00
Nodes (3): Live2D Cubism 示例模型, 桃濑日和 PRO, 名执 尽 PRO

### Community 367 - "manager-mobile pnpm workspace"
Cohesion: 0.67
Nodes (3): manager-mobile unibest 应用壳, @dcloudio/uni-h5 patch, manager-mobile pnpm workspace

### Community 372 - "manager-web Vue 应用壳"
Cohesion: 0.67
Nodes (3): manager-web CDN 注入, manager-web Vue 应用壳, 小智控制台离线模式

### Community 373 - "org.junit.jupiter.api.BeforeEach"
Cohesion: 0.05
Nodes (51): MessageUtils, Override, SpringContextUtils, AiAgentChatHistoryDao, AgentChatHistoryType, AGENT, USER, AgentService (+43 more)

### Community 375 - "Gemini LLMProvider._generate"
Cohesion: 0.67
Nodes (3): Gemini LLMProvider._generate, Gemini LLMProvider.response, Gemini LLMProvider.response_with_functions

### Community 376 - "WechatServiceImplProfileTest"
Cohesion: 0.10
Nodes (5): jakarta.validation.Validator, WechatProfileUpdateDTO, ProfileValidator, WechatProfileUpdateDTOTest, WechatServiceImplProfileTest

### Community 378 - "OpenAIStyleLLMServiceImpl"
Cohesion: 0.47
Nodes (3): HttpEntity, Override, OpenAIStyleLLMServiceImpl

### Community 379 - "WebSocketClientManager"
Cohesion: 0.13
Nodes (4): BinaryMessage, WebSocketClientManager, StopWatch, TextMessage

### Community 384 - "Result.java"
Cohesion: 0.24
Nodes (8): API Response Envelope Pattern, i18n Message Resolution, RenExceptionHandler.handleDuplicateKeyException, RenExceptionHandler.handleException, RenExceptionHandler.handleMethodArgumentNotValidException, RenExceptionHandler.handleNoResourceFoundException, RenExceptionHandler.handleRenException, RenExceptionHandler.handleUnauthorizedException

### Community 388 - "AgentSnapshotService"
Cohesion: 0.19
Nodes (6): AgentSnapshotService, AgentSnapshotRedactionRunner, Override, AgentSnapshotRedactionRunnerTest, org.springframework.beans.factory.SmartInitializingSingleton, org.springframework.boot.test.system.CapturedOutput

### Community 399 - "NfcWriteJobDialog.vue"
Cohesion: 0.30
Nodes (8): beforeDestroy(), dialogVisible(), fetchJobDetail(), jobId(), onImported(), startPolling(), stopPolling(), visible()

### Community 428 - "NfcManualWrite.vue"
Cohesion: 0.26
Nodes (10): manualWriteStatusLabel(), manualWriteStatusTagType(), confirmLock(), copyScheme(), created(), fetchAssets(), mark(), showSchemeForManualCopy() (+2 more)

### Community 430 - "NfcActivationManagement.vue"
Cohesion: 0.18
Nodes (6): statusLabel(), doActivate(), generateUUID(), handleActivate(), handleScanned(), statusText()

### Community 431 - "PdcNfcLiquibaseIntegrationTest.java"
Cohesion: 0.25
Nodes (5): java.sql.Connection, JdbcTemplate, PdcNfcLiquibaseIntegrationTest, org.junit.jupiter.api.AfterAll, org.springframework.jdbc.core.JdbcTemplate

### Community 432 - "ChunkMethod"
Cohesion: 0.10
Nodes (35): BatchIdReq, ChunkMethod, BOOK, EMAIL, KNOWLEDGE_GRAPH, LAWS, MANUAL, NAIVE (+27 more)

### Community 434 - "BCrypt"
Cohesion: 0.14
Nodes (6): java.security.SecureRandom, InviteCodeGenerator, BCrypt, BCryptPasswordEncoder, Override, org.apache.commons.logging.Log

### Community 435 - "Prototype"
Cohesion: 0.50
Nodes (3): Prototype, PetCollectionCardProperties.randomCollectionCardUrl, PetCollectionCardProperties.selectConfig

### Community 438 - "AgentTemplateService"
Cohesion: 0.18
Nodes (4): ResultUtils, AgentTemplateController, PageData, AgentTemplateService

### Community 524 - "iE"
Cohesion: 0.16
Nodes (8): Dl(), fi, iE(), rE(), sa(), sd, $w(), xB

### Community 527 - "localizedSnapshotDisplayValue"
Cohesion: 0.50
Nodes (5): escapeHtml(), formatFunctionParamValue(), localizedSnapshotDisplayValue(), renderInlineMarkdown(), renderMarkdownValue()

### Community 528 - "PdcNfcBatchAdminController.java"
Cohesion: 0.18
Nodes (7): GetMapping, PostMapping, RequestMapping, RestController, PdcNfcBatchAdminController, PdcNfcBatchQueryDTO, PdcNfcBatchService

### Community 529 - "nfc-claim-api.test.js"
Cohesion: 0.22
Nodes (5): assert, calls, Module, nfcClaimApi, responses

### Community 530 - "pet-api.test.js"
Cohesion: 0.22
Nodes (5): assert, calls, Module, petApi, responses

### Community 534 - "com.fasterxml.jackson.databind.ObjectMapper"
Cohesion: 0.10
Nodes (14): com.fasterxml.jackson.databind.JsonNode, com.fasterxml.jackson.databind.ObjectMapper, PdcNfcAdminOperationType, ACTIVATE, DISABLE, SCRAP, STOCK_IN, WRITE_RESULT_IMPORT (+6 more)

### Community 535 - "PdcNfcManualMarkAction"
Cohesion: 0.20
Nodes (6): PdcNfcManualMarkAction, MARK_LOCKED, MARK_VERIFIED, MARK_WRITE_FAILED, MARK_WRITTEN, PdcNfcManualAssetVO

### Community 536 - "NfcBatchDialog.vue"
Cohesion: 0.36
Nodes (5): fetchProductTypes(), fetchPrototypes(), handleClose(), handleSubmit(), visible()

### Community 537 - "KnowledgeBaseDTO"
Cohesion: 0.21
Nodes (3): KnowledgeBaseController, KnowledgeBaseDTO, KnowledgeManagerService

### Community 538 - "knowledgeBase.js"
Cohesion: 0.27
Nodes (13): createKnowledgeBase(), deleteDocument(), deleteKnowledgeBase(), deleteKnowledgeBases(), getAuthToken(), getDocumentList(), getKnowledgeBaseList(), listChunks() (+5 more)

### Community 539 - "RAGFlowClient"
Cohesion: 0.31
Nodes (3): ObjectMapper, RestTemplate, RAGFlowClient

### Community 540 - "ChunkDTO"
Cohesion: 0.41
Nodes (11): AddReq, ChunkDTO, InfoVO, AllArgsConstructor, Data, NoArgsConstructor, ListReq, ListVO (+3 more)

### Community 541 - "FieldMetaObjectHandler.java"
Cohesion: 0.48
Nodes (4): com.baomidou.mybatisplus.core.handlers.MetaObjectHandler, FieldMetaObjectHandler, Override, org.apache.ibatis.reflection.MetaObject

### Community 545 - "PdcNfcPrototype.java"
Cohesion: 0.33
Nodes (3): PdcNfcPrototype, KOI, RABBIT

### Community 546 - "KnowledgeFilesService"
Cohesion: 0.20
Nodes (3): InfoVO, ListReq, KnowledgeFilesService

### Community 547 - "TokenServiceImpl.java"
Cohesion: 0.27
Nodes (4): TokenGenerator, Override, TokenServiceImpl, TokenService

### Community 548 - "DeviceOtaVO"
Cohesion: 0.22
Nodes (5): Activation, DeviceOtaVO, Firmware, Mqtt, ServerTime

### Community 560 - "ASRProvider"
Cohesion: 0.28
Nodes (5): ASRProvider, Convert speech data to text using FunASR. :param opus_data: List of Opus-…, Initialize the ASRProvider with server configuration. :param config: Dictionary…, Asynchronous generator to receive messages from the WebSocket. Yields each…, Internal method to handle WebSocket communication. Reuses the persistent…

### Community 561 - "TTSPerformanceTester"
Cohesion: 0.39
Nodes (3): create_instance(), main(), TTSPerformanceTester

### Community 562 - "soul-resonance.js"
Cohesion: 0.38
Nodes (5): CHARACTERS, _checkCanProceed(), onNext(), onQuirkTap(), onTraitTap()

### Community 565 - "PdcNfcProductTypeAdminController"
Cohesion: 0.40
Nodes (4): PostMapping, RequestMapping, RestController, PdcNfcProductTypeAdminController

### Community 566 - "handleDrag"
Cohesion: 0.50
Nodes (5): handleDrag(), handleProgressClick(), seekToPercentage(), startDrag(), stopDrag()

## Knowledge Gaps
- **1836 isolated node(s):** `_common.sh script`, `start.sh script`, `auth`, `{ post }`, `{ captureNfcClaimIntent }` (+1831 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **226 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `error()` connect `error` to `DeviceManagement.vue`, `login/index.vue`, `agent/index.vue`, `alova.ts`, `AgentSnapshotPanel.vue`, `httpRequest.js`, `HeaderBar.vue`, `edit.vue`, `VoiceCloneDialog.vue`, `t`, `settings/index.vue`, `index/index.js`, `TtsModel.vue`, `doodle-api.js`, `register/index.vue`, `tools.vue`, `Alova HTTP Client`, `device/index.vue`, `CustomDialog.vue`, `roleConfig.vue`, `voice-call.js`, `detail.vue`, `subscription.js`, `FeatureManager`, `FeatureManagement.vue`, `home.vue`, `ultrasonic-config.vue`, `KnowledgeBaseManagement.vue`, `VoiceCloneManagement.vue`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Why does `saveArtwork()` connect `doodle-api.js` to `doodle-editor.js`, `error`, `pet-store.js`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `setup_logging()` connect `setup_logging` to `MCPClient`, `VADProviderBase`, `TTSProviderBase`, `ActionResponse`, `TextMessageType`, `MCPEndpointClient`, `ConnectionHandler`, `GlobalCacheManager`, `connection.py`, `ServerMCPClient`, `MemoryProviderBase`, `xiaozhi-server/app.py`, `PromptManager`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Are the 4 inferred relationships involving `Result` (e.g. with `.onAccessDenied()` and `.onLoginFailure()`) actually correct?**
  _`Result` has 4 INFERRED edges - model-reasoned connections that need verification._
- **What connects `_common.sh script`, `start.sh script`, `auth` to the rest of the system?**
  _1836 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Result` be split into smaller, more focused modules?**
  _Cohesion score 0.037875696994424043 - nodes in this community are weakly interconnected._
- **Should `index-Guo1hQ-y.js` be split into smaller, more focused modules?**
  _Cohesion score 0.00870751458508693 - nodes in this community are weakly interconnected._