...
class ConnectionHandler:
    ...
    async def _process_mqtt_audio_message(self, message):
        ...
        try:
            ...
            # 基于时间戳进行乱序重排后再入队
            self._process_websocket_audio(pcm_frame, timestamp)
            return True
        except Exception as e:
    ...
    def _process_websocket_audio(self, audio_data, timestamp):
        """基于时间戳对音频包进行乱序重排，再经AEC处理后送入ASR队列"""
        # 如果时间戳是递增的，直接处理
        if timestamp >= self.last_processed_timestamp:
            self._put_asr_audio_with_aec(audio_data, timestamp)
            ...
            while processed_any:
                ...
                for ts in sorted(self.audio_timestamp_buffer.keys()):
                    if ts > self.last_processed_timestamp:
                        ...
                        self._put_asr_audio_with_aec(buffered_audio, ts)
    ...
    def _put_asr_audio_with_aec(self, pcm_frame, timestamp):
        """对PCM帧执行AEC处理（如果启用），然后放入ASR队列"""
