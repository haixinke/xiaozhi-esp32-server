...
| 亲密度道具展示 | **展示**，但标注「亲密度效果将在后续版本生效」 | 后端亲密度增长引擎尚未接入（见后端方案 §10）；道具仍可购买并计入库存 |
| 外观重复购买 | **不可重复** | `outfit` 已解锁（`remainCount>0`）后购买按钮替换为「去换装」 |
...
分类显示顺序固定为：`变更券 → 声音 → 外观 → 亲密度礼物`（不由后端 `sort` 决定大类顺序；大类内按 `sku.sort` 升序）。分类 → 中文标签映射：
...
- 底部备注：`券类道具数量固定为 1；礼物的亲密度效果将在后续版本生效`
...
| 图标分类底色 | 变更券=粉系、声音=紫系、外观=粉系、礼物=金系（均 12%→4% 渐变） |
...
- DB：道具 SKU 由后端迁移维护；本次新增的 `voice_change`（`202606151832.sql`）会自动出现在「变更券」分类，¥99。

---

## 13. 待评审确认项汇总

1. **券类囤积**：已拥有的券是否仍允许购买（默认：允许，固定 1 张/次）？
2. **亲密度道具**：在引擎未接入前是否展示并允许购买（默认：展示 + 购买，标注后续生效）？
3. **「去换装」目标**：外观选择页是否就绪？未就绪时本期内是占位 toast 还是先不上该按钮？
4. **取消支付**：用户在微信侧取消后，是否主动调 `/payment/order/{outTradeNo}/cancel`（默认：不调，依赖后端 15 分钟自动超时关单）？
...
- [ ] 页面展示后端返回的全部有效道具，按 变更券/声音/外观/亲密度礼物 四类分段、类内按 `sort` 排序。
